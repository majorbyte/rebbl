...
const OptionsType = function(options) {
  
    options.uri = options.uri || 'mongodb://127.0.0.1:27017/test';
    options.collection = options.collection || 'sessions';
    //options.connectionOptions = options.connectionOptions || null;
    options.expires = options.expires || 1000 * 60 * 60 * 24 * 14; // 2 week
    options.idField = options.idField ||'_id';
    options.databaseName = options.databaseName ||  null;
    options.expiresKey = options.expiresKey || 'expires';
    options.expiresAfterSeconds = options.expiresAfterSeconds || 0;
  return options;
};
