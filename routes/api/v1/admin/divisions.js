...
const express = require("express")
  ...
  router.get("/", util.ensureAuthenticated, util.hasRole("admin"), async function(req, res){
    try{
      ...
      let college = await dataService.getSchedules({season:/season 19/i, round:1, league:"ReBBRL College League"},{projection:{_id: 0,league:1, competition:1, competition_id:1}});
  ...
  router.get("/bb3", util.ensureAuthenticated, util.hasRole("admin"), async function(req, res){
    try{
      ...
      let data = await dataServiceBB3.getCompetitions({leagueId:"94f0d3aa-e9ba-11ee-a745-02000090a64f",season:"season 4", status:{$lt:4}});

      res.status(200).send(data.filter(x => !x.isParent).sort((a,b) => a.name > b.name ? 1 : -1));
    } catch(err){
      console.log(err); 
    }
  });
