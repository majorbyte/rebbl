...
const express = require("express")
  ...
  router.get("/", util.ensureAuthenticated, util.hasRole("admin"), async function(req, res){
    try{
      ...
      let data = await dataService.getSchedules({season:"season 24", round:1, league:{$in:["REBBL - GMan","REBBL - Big O", "REBBL - REL","REBBL - REL 2","REBBL - GMan 2"]}},{projection:{_id: 0,league:1, competition:1, competition_id:1}});
      let upstarts = await dataService.getSchedules({season:/season 24/i, round:1, league:"ReBBRL Upstarts"},{projection:{_id: 0,league:1, competition:1, competition_id:1}});
      ...
      let playoffs = await dataService.getSchedules({season:"season 24", round:1, league:"ReBBL Playoffs",competition:{$in:['Challenger\'s Cup XXI','REBBL Playoffs Season 21']}},{projection:{_id: 0,league:1, competition:1, competition_id:1}});
