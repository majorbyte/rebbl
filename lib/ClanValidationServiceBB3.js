...
class ClanValidationService {
  constructor(){
    ...
    this.tierTax = [
      ...
      {race:"underworldDenizen", tier:1, cost:100_000},
  ...
  checkFreshTeamBasicRules(teams){
    ...
    return teams.map(team => team.roster?.length >= 11 ? '' : team.roster ? `Rule 1.3: ${team.name} needs at least 11 players` : '')
      .filter(x => x !== '');
  }
  ...
  playerCost(player) {
    ...
    const cost = player.playerType.cost + this.getSkillCost(player) + (player.skill2 ? 20000 : 0 )
    return cost;
  }
  ...
  teamCost = (team) => {
    return this.rosterCost(team) + this.subTotalRerolls(team);
  } 
