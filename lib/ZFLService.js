...
class ZFLService{
  ...
  async updateSchedules (competition){
    ...
    const nameRegex = new RegExp(`${competition.name}`, "i");
    const zflCompetition = await dataService.getZFLCompetition({"fixtures.competition.name": nameRegex, year: this.year});
  ...
  async #updateTeamStatsStandings(){
    ...
    for(const team of data){
      ...
      await dataService.updateZFLStats({statsType:team.statsType, TeamId: team._id}, team,{upsert:true});
    }
  ...
  async ReleaseAndUpdate(matchId){
    ...
    if (match.competition.name.toLowerCase().indexOf("howitzer") === -1) await this.#updatePlayers(match.homeTeam.statistics, match.homeTeam.id);
    if (match.competition.name.toLowerCase().indexOf("howitzer") === -1) await this.#updatePlayers(match.awayTeam.statistics, match.awayTeam.id);
