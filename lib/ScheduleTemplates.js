...
class ScheduleTemplates {
  ...
  async getBB3Divisions(league, prefix,round){
    ...
    let divisions = await dataServiceBB3.getCompetitions({leagueId:league, season:"season 4"});
  ...
  async divisionLineBB3(division, round){
    return  `\n  [${division.name}](https://rebbl.net/competition/${encodeURIComponent(division.id)}/schedules/${round})    `;
  }
}
