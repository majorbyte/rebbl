...
class CyanideService{
  ...
  async endpoints(){
    try{
      let response = await axios.get(`http://web.cyanide-studio.com/ws/?key=${this.apiKey}`);
      if (response.data.meta)
        delete response.data.meta;
      if (response.data.urls)
        delete response.data.urls;
   ...
   * @return {ReturnContestMatch} list of matches
   */
   ...
   * @param {number} [params.limit] - Max amount of results (default = 100)
   * @param {number} [params.exact] - 0|1 : Exact league name match
   * @return {ReturnCompetitions} list of competitions
   */
   ...
   * @param {string} [params.league] - League name (default = Cabalvision Official League)
   * @param {string} [params.platform] - pc|ps4|xb1
   * @return {ReturnLeagueCheck} information about the league 
   */
  async league(params){
   ...
   * @return {ReturnMatchData} list of matches
   */
   ...
   * @param {string} [params.league] - League name (default = Cabalvision Official League)
   * @param {string} [params.platform] - pc|ps4|xb1
   * @return {string} Latest time a match was played
   */
