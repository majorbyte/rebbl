...
class CyanideService{
  ...
  async _getData(endpoint, params){
    ...
    while(x < 3){
      try {
        x++;
        let response = await fetch(`${this._url(endpoint)}&${queryString}`);
  ...
  async endpoints(){
    try{
      let response = await fetch(`https://web.cyanide-studio.com/ws/?key=${this.apiKey}`);
  ...
  /**
   * 
   * @param {Object} params - The lookup object
   * @param {string} [params.platform_name] -  pc|playstation|xbox
   * @param {string} [params.opus] - Opus 1|2|3
   * @param {string} [params.league_name] - League name
   * @param {string} [params.league_id] - League ID
   * @param {string} [params.sort] - ID|LastMatchDate|CreationDate
   * @param {string} [params.competition_name] - Competition name
   * @param {string} [params.competition_id] - Competition ID
   * @param {string} [params.team_name] - Team name
   * @param {string} [params.team_id] - Team ID
   * @param {string} [params.coach_name] - Coach name
   * @param {string} [params.coach_id] - Coach ID
   * @param {string} [params.exact] - Exact league name match 0|1
   * @param {string} [params.hint] - Lookup instruction
   * @param {string} [params.fallback] - Fallback to defaults if nothing found
   * @return {Promise<Object>} the result of the lookup
   */
  lookup = async params => this._getData("lookup", params);
}
