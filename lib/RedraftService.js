...
class RedraftService{
  ...
  async startRedraft(teamId, user){
    ...
    for(const player of redraft.roster){ 
      ...
      const seasons = [...new Set(competitions.filter(x => competitionIds.indexOf(x.id) > -1 ).map(x => x.season) )].filter(x => x);
  ...
  async #calculateFunds(teamId, treasury){
    ...
    let competitions = await dataService.getCompetitions({"standings.teamId":teamId, season:"season 2", parentId:{$exists:0},format:{$ne:3}}); // TODO: adjust for season
    let offseasonCompetitions = await dataService.getCompetitions({"standings.teamId":teamId, season:"season 2", name:{$in:["Ure Trophy","Greenhorn Cup"]}}); // TODO: adjust for season
