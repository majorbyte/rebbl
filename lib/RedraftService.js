...
class RedraftService{
  ...
  async startRedraft(teamId, user){
    ...
    if (!data.responseGetTeamCompetitions.competitions.competition) throw {error: "The team is not part of the ReBBL Redraft competition, please join this competition ingame."};
  ...
  async #calculateFunds(teamId, treasury){
    ...
    let competitions = await dataService.getCompetitions({"standings.teamId":teamId, season:"season 3", excludeRedraft:false}); // TODO: adjust for season
