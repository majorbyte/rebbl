...
class RedraftService{
  ...
  async checkRedraft(teamId, user){
    ...
    team.wrongCompetition = response.data.responseGetTeamCompetitions.competitions.competition.id !== this.competitionId;
  ...
  async validateDraft(teamId, user){
    ...
    for (const p of roster){
      if (getDateFromUUID(p.player.id) > team.redraft.confirmedOn) {
        // new rookie
        const index = team.redraft.allowedPositions.findIndex(x => x.type == p.player.position); // '23' vs 'thrower'
