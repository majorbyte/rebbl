...
class RedraftService{
  ...
  async confirmDraft(teamId, user){
    ...
    for(const cas of player.casualties.filter(c => c.old !== c.new)){
      ...
      if (c.new > 0) await adminBB3Service.setInjuryInjury(cookie, player.id, c.new);
    }
    await adminBB3Service.clearMng(cookie, roster.filter(x => x.selected && x.casualties.any(cas => cas.dice > 0)));
    ...
    await dataService.updateTeam({id:teamId},{$set:{"team.redraft.status": "confirmed", "team.redraft.confirmedOn": Date.now()}});
  }
  ...
  async validateDraft(teamId, user){
    ...
    for (const p of roster){
      if (getDateFromUUID(p.player.id) > team.redraft.confirmedOn) {
        // new rookie
        const index = team.redraft.allowedPositions.findIndex(x => x.type == p.player.position);
        ...
        if (!draftPlayer.selected) errors.push({error: `Player ${draftPlayer.name} should be sacked.`}); 
      }
    }
    ...
    await dataService.updateTeam({id:teamId},{$set:{"team.redraft.status": "validated", "team.redraft.validatedOn": Date.now()}});
  }
