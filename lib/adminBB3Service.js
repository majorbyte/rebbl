...
class AdminBB3Service{
  ...
  async setApothecary(cookie, quantity){
    ...
    this.#assignImprovement(cookie, "set_apothicary", "bb3_set_apothicary_Apothicary", quantity)
  }

  async setCheerleaders(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#assignImprovement(cookie, "set_cheerleaders", "bb3_set_cheerleaders_Cheerleaders", quantity)
  }

  async setAssistantCoaches(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#assignImprovement(cookie, "set_coach_assistants", "bb3_set_coach_assistants_CoachAssistants", quantity)
  }

  async setRerolls(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#assignImprovement(cookie, "set_rerolls", "bb3_set_rerolls_Rerolls", quantity)
  }

  async setDedicatedFans(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#assignImprovement(cookie, "set_dedicated_fans", "bb3_set_dedicated_fans_DedicatedFans", quantity)
  }
  ...
  async #assignImprovement(cookie,query,fieldName, value){
    const data = new Map();
