...
class AdminBB3Service{
  ...
  async clearMng(cookie, players){
    ...
    for(const player of players){
      this.#setMngState(cookie, player.id, "false");
    }
    return cookie;
  }
  ...
  async applyMng(cookie, players){
    ...
    for(const player of players){
      this.#setMngState(cookie, player.id, "true");
    }
    return cookie;
  }
  ...
  async setApothecary(cookie, quantity){
    ...
    this.#executeQuery(cookie, "set_apothicary", "bb3_set_apothicary_Apothicary", quantity)
  }

  async setCheerleaders(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#executeQuery(cookie, "set_cheerleaders", "bb3_set_cheerleaders_Cheerleaders", quantity)
  }

  async setAssistantCoaches(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#executeQuery(cookie, "set_coach_assistants", "bb3_set_coach_assistants_CoachAssistants", quantity)
  }

  async setRerolls(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#executeQuery(cookie, "set_rerolls", "bb3_set_rerolls_Rerolls", quantity)
  }

  async setDedicatedFans(cookie, quantity){
    if (cookie == null) cookie = await this.#login();

    this.#executeQuery(cookie, "set_dedicated_fans", "bb3_set_dedicated_fans_DedicatedFans", quantity)
  }

  async setInjury(cookie, playerId, injury){
    if (cookie == null) cookie = await this.#login();

    this.#executeQuery(cookie, "add_injury", "bb3_add_injury_Player", playerId, "bb3_add_injury_Injury", injury)
  ...
  async removeInjury(cookie, playerId, injury){
    ...
    this.#executeQuery(cookie, "remove_injury", "bb3_remove_injury_Player", playerId, "bb3_remove_injury_Injury", injury)
