...
class ConfigurationService {
  ...
  getSeasons () {return this.seasons.concat(this.oneMinuteSeasons).concat(this.linemanSeasons).concat(this.elflySeasons)};
  ...
  async getArticles(){
    ...
    while(m){
      ...
      let article = {
        ...
        title: title.exec(m[1])[1].replace(/\t/g,'').replace(/\n/g,'').replace(/&nbsp;/g,' '),
        text: text.exec(m[1])[1].replace(/\t/g,'').replace(/\n/g,'').replace(/&nbsp;/g,' ').slice(0,-2),
