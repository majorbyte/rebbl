...
class TicketService{
  ...
  async checkTickets(){
    const data = [
      /*{id:42290, season:"Season 14"},
      {id:42291, season:"Season 14"},
      {id:42292, season:"Season 14"},
      {id:75681, season:"Season 14"},
      {id:75684, season:"Season 14"},*/
      {id:65181, season:"Season 10"},
      {id:65390, season:"Season 11"}
    ];
  ...
  async _processTicket(ticket,season){
    ...
    const standing = await dataService.getStanding({id:coachId, season:season.toLowerCase()});
    if (!standing){
      let score = this._newScore(season, coachId, ticket.Coach.User,ticket.RowLeague.Name,ticket.RowCompetition.Name,Number(ticket.RowCompetition.Id.Value.replace(/\D/g,"")), 
        ticket.RowTeam.Name,ticket.RowTeam.ID.Value.replace(/\D/g,""),Number(ticket.RowTeam.IdRaces));
      dataService.updateStanding({id:coachId, season:season.toLowerCase()}, score,{upsert:true});
    }
  }
  ...
  async _processParticipant(participant, league, competition,season){
    ...
    let standing = await dataService.getStanding({id:coachId, season:season.toLowerCase()});
    if (!standing){
      standing = this._newScore(season, coachId, participant.NameCoach, league,competition,Number(participant.IdCompetition.Value.replace(/\D/g,"")), 
    ...
    dataService.updateStanding({id:coachId, season:season.toLowerCase()}, standing,{upsert:true});
  }
