...
class TicketService{
  ...
  async checkTickets(){
    const data = [
      {id:42290, season:"Season 24"},
      {id:42291, season:"Season 24"},
      {id:42292, season:"Season 24"}/*,
      {id:75681, season:"Season 24"},
      {id:75684, season:"Season 24"},
  ...
  async _checkLeague(leagueId,season){
    const league = await apiService.getLeagueInfo(leagueId).catch(err => console.trace(err));
    if (!league) return;

    league.ResponseGetLeagueData.LeagueData.Competitions.CompetitionSummaryData.forEach(x => this._getTickets(x,season),this);
  }
  ...
  async _getTickets(competition,season){
    if(competition.Row.Name.startsWith(season)){
      ...
      let info = await apiService.getCompetitionTicketInfo(competitionId).catch(err => console.trace(err));
      if (!info) return;
      let c = await apiService.getCompetitionInfo(competitionId).catch(err => console.trace(err));
      if (!c) return;
