...
class HugeService{
  ...
  newScore(season, coach,team, points, tdDiff){
    ...
    return {
        ...
        competition: "HJ Memorial Ladder",
        season: season,
        team: team.teamname,
  ...
  async calculateStandings(season){
    ...
    */
    ...
    await Promise.each(matches, async match => {
      ...
      let homeCoach = this._getCoach(coaches,0,season,match.match);
      let awayCoach = this._getCoach(coaches,1,season,match.match);
    ...
    coaches = coaches.sort(
      function(a,b){
        ...
        if (a.bigGuy && b.bigGuy){
          if (a.bigGuy.spp > b.bigGuy.spp) {return -1;} 
          if (b.bigGuy.spp > a.bigGuy.spp) {return 1;} 
        } 

        if (a.bigGuy && !b.bigGuy) {return -1;} 
        if (b.bigGuy && !a.bigGuy) {return 1;} 
  ...
  _getCoach(coaches, index, season, match){
    let coach = coaches.find(function (c) {
      return c.id === match.coaches[index].idcoach && c.teamId === match.teams[index].idteamlisting;
    });

    if (!coach) {
      coach = this.newScore(season,match.coaches[index],match.teams[index],0,0); 
      coaches.push(coach);
    }
  ...
  async _processRoster(teamId){
    ...
    let activePlayers = await dataService.getPlayers({teamId:teamId,active:true});
