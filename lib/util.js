...
module.exports = {
  ...
  cache: (duration, noRender) => {
    return (req, res, next) => {
	  ...
	  if (noRender) res.send(cachedBody);
          else res.render("no-cache-relayout",{data:cachedBody}, function(error, html){
            res.send(html);
          });
        }
        return;
        ...
        if (key.indexOf("/api") > -1){
          ...
          res.send = (body) => {
            cache.put(key, body,duration*1000);
	          if(noRender) res.sendResponse(body);
	          else res.render("no-cache-relayout",{data:body}, function(error, html){res.sendResponse(html);});
          };
        }
        next();
      }
  ...
  ensureAuthenticated: function (req, res, next) {
    try{
      ...
      req.session.returnUrl =  `${req.protocol}://${req.get("Host")}${req.originalUrl}` //req.originalUrl;
  ...
  checkBadBots: function(req,res,next){
    ...
    if(agent && agent.indexOf("BLEXBot")>-1) return res.redirect(301,"http://webmeup-crawler.com/");
    else if(agent && agent.indexOf("Baiduspider") >-1) return res.redirect(301, "http://www.baidu.com/search/spider.html");
    else if(agent && agent.indexOf("TweetmemeBot") >-1) return res.redirect(301, "https://datasift.com/bot.html");
    else return next();    
  },

  checkTeapots: function(req,res,next){
    const teapots = ["svc/shreddit/", ".php", "wp-includes/","wp-admin/","wp-json/",".git","wp-content/",".vscode",".env","com_acym/","webtools/","aspera/"];
    const teapotAgents = ["l9explore", "custom-asynchttpclient", "zgrab", "keydrop", "internal dummy"];
    if (teapots.some(teapot => req.originalUrl.indexOf(teapot) > -1) ) return res.status(418).send("I'm a teapot");
    
    let agent = req.headers['user-agent'];
    if (agent && teapotAgents.some(teapot => agent.toLowerCase().indexOf(teapot) > -1) ) return res.status(418).send("I'm a teapot");
    else return next();    
  },
  ...
  verifyMaintenanceHeader:function(req, res, next) {
    return req.get('verificationHeader') === process.env['verificationHeader'] ? next() : res.status(403).send();
  },
