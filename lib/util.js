...
module.exports = {
  ...
  getISOWeek: function() {
    ...
    day = (day >= 0 ? day : day + 7);
    const daynum = Math.floor((now.getTime() - newYear.getTime() - 
    (now.getTimezoneOffset()-newYear.getTimezoneOffset())*60000)/86400000) + 1;
    let weeknum;
    //if the year starts before the middle of a week
    if(day < 4) {
        weeknum = Math.floor((daynum+day-1)/7) + 1;
        if(weeknum > 52) {
            let nYear = new Date(now.getFullYear() + 1,0,1);
