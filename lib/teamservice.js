...
class TeamService{
  ...
  async updateTeam(teamId){
    ...
    await Promise.all(team.roster.map(async function(currentPlayer){
      ...
      if(player){
        let skills = currentPlayer.skills.length > 0 ? currentPlayer.skills : this.allSkills[currentPlayer.type];


        await this.players.update({_id:player._id},{$set:{"active":true, "skills":skills}});
      } else {
