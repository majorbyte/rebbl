...
class TeamService{
  ...
  async updateTeams(id, justTeam){
      ...
      if (justTeam){
        ...
        teams = await dataService.getTeams({$and:[{'team.name':/^(?!.+\S19$).+$/},{"team.datelastmatch":{$gt:dateLastMatch}}]});
        let clanTeams = await dataService.getTeams({'team.name':/^(?!.*proxy).*(S19)$/i});
    ...
    if (!justTeam){    
      ...
      await Promise.all(currentTeam.roster.map(async function(currentPlayer){
        ...
        if(player && currentPlayer.id !== null){
          ...
          dataService.updatePlayer({_id:player._id},{"teamId":teamId, "active":true, "skills":skills,"attributes":currentPlayer.attributes, "value":currentPlayer.value, "xp": currentPlayer.xp, "level":currentPlayer.level, "number":currentPlayer.number, casualties_state: currentPlayer.casualties_state, casualties_state_id: currentPlayer.casualties_state_id,suspended_next_match:currentPlayer.suspended_next_match});
