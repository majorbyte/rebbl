...
class TeamService{
  ...
  async getTeamStats(_id){
    ...
    let team;
    if (isNaN(teamId)){
      const regex = new RegExp(_id.trim().replace('?','\\?'), 'i');
      team = await this.teams.findOne({"team.name":{ $regex: regex }});
      teamId = team.team.id;
    } else {
  ...
  async updateTeam(teamId){
    ...
    await Promise.all(team.roster.map(async function(currentPlayer){
      ...
      if(player && currentPlayer.id !== null){
        ...
        await this.players.update({_id:player._id},{$set:{"active":true, "skills":skills, "value":currentPlayer.value, "xp": currentPlayer.xp, "level":currentPlayer.level, "number":currentPlayer.number}});
      } else {
