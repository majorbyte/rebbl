  ...
  async updateTeam(teamId){
    ...
    await Promise.all(team.roster.map(async function(currentPlayer){
      ...
      if(player && currentPlayer.id !== null){
        ...
        await this.players.update({_id:player._id},{$set:{"active":true, "skills":skills, "value":currentPlayer.value}});
      } else {
      ...
      console.dir(player);
      if (player && player.casualties_sustained && ((player.casualties_sustained.length === 1 && player.casualties_state[0] !== "BadlyHurt") 
          || (player.casualties_sustained.length === 2 && player.casualties_state[0] !== "BadlyHurt" && player.casualties_state[1] !== "BadlyHurt")) ){
    ...
    console.log(lonerValue);
    currentTeam.team.nextMatchTV = currentTeam.team.value + (missingPlayers * lonerValue/1000);
