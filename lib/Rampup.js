...
class Rampup {
  ...
  async getCoachScore() {
    let coaches = [];
    let schedules = await dataService.getSchedules({"league": {"$regex": new RegExp(/rampup$/,"i")}, "status":"played", "match_uuid":{$gt:"10009cd431"}});
    ...
    schedules = await dataService.getSchedules({"competition": {"$regex": new RegExp(/^S17|Season 17|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"10009cd431"} } );
  ...
  async getAvailableRampupCoaches(){
    const signedUp = await dataService.getSignups({season:"season 17", "saveType":"rampup", exclude:{$ne:true}});
    let coaches = signedUp.map(x => x.coach);
    
    const schedules = await dataService.getSchedules({season:"season 17", "opponents.coach.name":{$in:coaches}, league:{$regex:/^REBBL -/i}});
    schedules.map(x => {
  ...
  async excluceCoach(coach){
    ...
    dataService.updateSignup({saveType:"rampup",season:"season 17", coach:{$regex: r}},{$set:{exclude:true}});
  }
