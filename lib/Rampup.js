...
class Rampup {
  ...
  async getCoachScore() {
    let coaches = [];
    let schedules = await dataService.getSchedules({"league": {"$regex": new RegExp(/rampup$/,"i")}, "status":"played", "match_uuid":{$gt:"1000a4b5d0"}});
    ...
    schedules = await dataService.getSchedules({"competition": {"$regex": new RegExp(/^S18|Season 18|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"1000a4b5d0"} } );
