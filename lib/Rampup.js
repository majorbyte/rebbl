...
class Rampup {
  ...
  async getCoachScore() {
    let coaches = [];
    let schedules = await dataService.getSchedules({"league": {"$regex": new RegExp(/rampup$/,"i")}, "status":"played", "match_uuid":{$gt:"1000adb48c"}});
    ...
    schedules = await dataService.getSchedules({"competition": {"$regex": new RegExp(/^S19|Season 19|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"1000adb48c"} } );
  ...
  async getAvailableRampupCoaches(){
    const signedUp = await dataService.getSignups({season:"season 19", "saveType":"rampup", exclude:{$ne:true}});
    let coaches = signedUp.map(x => x.coach);
    
    const schedules = await dataService.getSchedules({season:"season 19", "opponents.coach.name":{$in:coaches}, league:{$regex:/^REBBL -/i}});
    schedules.map(x => {
  ...
  async createSchedule(isGMan, competitionName, competitionId,teams){
    //REL S19 W11 GAME 4
    let parts = competitionName.split(" ");

    const round = parseInt(parts[2].replace("W",""));
    const game = parseInt(parts[4]);
  ...
  async excluceCoach(coach){
    ...
    dataService.updateSignup({saveType:"rampup",season:"season 19", coach:{$regex: r}},{$set:{exclude:true}});
  }
