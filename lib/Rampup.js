...
class Rampup {
  ...
  async getCoachScore() {
    ...
    schedules = await dataService.getSchedules({"competition": {"$regex": new RegExp(/^S15|Season 15|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"1000790140"} } );
  ...
  async getAvailableRampupCoaches(){
    const signedUp = await dataService.getSignups({season:"season 15", "saveType":"rampup", exclude:{$ne:true}});
    let coaches = signedUp.map(x => x.coach);
    
    const schedules = await dataService.getSchedules({season:"season 15", "opponents.coach.name":{$in:coaches}, league:{$regex:/^REBBL -/i}});
    schedules.map(x => {
  ...
  async excluceCoach(coach){
    ...
    dataService.updateSignup({saveType:"rampup",season:"season 15", coach:{$regex: r}},{$set:{exclude:true}});
  }
