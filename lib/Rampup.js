...
class Rampup {
    ...
    async getCoachScore() {
        let schedules = await this.leagueService.searchLeagues({"league": {"$regex": new RegExp(/rampup$/,"i")}, "status":"played"});
        ...
        ids.splice(ids.indexOf(111960),1 );
        ids.splice(ids.indexOf(null),1 );

        schedules = await this.leagueService.searchLeagues({"competition": {"$regex": new RegExp(/^Season 9|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}} );
