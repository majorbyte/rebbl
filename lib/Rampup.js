...
class Rampup {
  ...
  async getCoachScore() {
    let coaches = [];
    let schedules = await this.leagueService.searchLeagues({"league": {"$regex": new RegExp(/rampup$/,"i")}, "status":"played", "match_uuid":{$gt:"1000635564"}});
    ...
    schedules = await this.leagueService.searchLeagues({"competition": {"$regex": new RegExp(/^Season 13|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"100054a72d"} } );
