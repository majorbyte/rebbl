...
class Rampup {
  ...
  async getCoachScore() {
    ...
    schedules = await this.leagueService.searchLeagues({"competition": {"$regex": new RegExp(/^Season 10|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"100054a72d"} } );
