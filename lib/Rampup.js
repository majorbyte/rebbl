...
class Rampup {
  ...
  async getCoachScore() {
    let coaches = [];
    let schedules = await this.leagueService.searchLeagues({"league": {"$regex": new RegExp(/rampup$/,"i")}, "status":"played", "match_uuid":{$gt:"10006daae0"}});
    ...
    schedules = await this.leagueService.searchLeagues({"competition": {"$regex": new RegExp(/^S13|Season 13|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"10006d685d"} } );
