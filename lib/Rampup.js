...
class Rampup {
  ...
  async getCoachScore() {
    let coaches = [];
    let schedules = await dataService.getSchedules({"league": {"$regex": new RegExp(/rampup$/,"i")}, "status":"played", "match_uuid":{$gt:"1000857000"}});
    ...
    schedules = await dataService.getSchedules({"competition": {"$regex": new RegExp(/^S16|Season 16|rampup$/,"i")}, "status":"played", "opponents.coach.id": {$in:ids}, "opponents.coach.name":{$ne:"Mystaes"}, "match_uuid":{$gt:"100090b787"} } );
  ...
  async getAvailableRampupCoaches(){
    const signedUp = await dataService.getSignups({season:"season 16", "saveType":"rampup", exclude:{$ne:true}});
    let coaches = signedUp.map(x => x.coach);
    
    const schedules = await dataService.getSchedules({season:"season 16", "opponents.coach.name":{$in:coaches}, league:{$regex:/^REBBL -/i}});
    schedules.map(x => {
  ...
  async excluceCoach(coach){
    ...
    dataService.updateSignup({saveType:"rampup",season:"season 16", coach:{$regex: r}},{$set:{exclude:true}});
  }
