...
class ConcedeMatchService{
  ...
  async concedeMatch(matchId, winningTeamId){
    ...
    if (winningTeamId == homeTeam.id) {
      ...
      await this.#awardSpp(cookie, homeTeam);
      homeWinnings = await this.#awardCash(cookie, homeTeam, awayDF);
      homeWinnings.score = 1;
    } else {
      cookie = await adminService.setTeamToAdmin(cookie, competition.id, awayTeam.id);
      await this.#awardSpp(cookie, awayTeam);
      awayWinnings = await this.#awardCash(cookie, awayTeam, homeDF);
  ...
  async #awardCash(cookie, team, otherDedicatedFans){
