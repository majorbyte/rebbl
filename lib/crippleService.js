...
class CrippleService{
  constructor(){
    ...
    this.wss.on('connection', async function (ws) {
      ...
      let data = await this.getCasualties();
      
      if(ws.OPEN === ws.readyState) ws.send(JSON.stringify(data));
    
      /*setInterval(
        () => ws.readyState === ws.OPEN ? ws.send(x++) : false,
        5000
      )*/
    }.bind(this) );
  ...
  async getMatch(id){
    try {
      ...
      let exist = await this.dataService.getMatch({"uuid": id});
      ...
      if (match){
        await this.dataService.insertMatch(match);

        await this.updateTeamsAfterMatch(match);
        
        cache.del(encodeURI(`/cripple`));
      }
    } catch(e) {
      console.log(id);
  ...
  async getMatches(){
    try {
      ...
      const league = await cyanideService.league({platform:"pc",league:"REBBL Cripple Ladder"});

      //let now = new Date(Date.now());

      let now = new Date('2019-02-11');
      ...
      console.log(lastMatch.toISOString());

      //if (lastMatch < now)return
      now.setHours(now.getHours()-2);


      const ids = await cyanideService.matches({league:"REBBL Cripple Ladder", start: now.toISOString(),id_only:1,limit:2000})

      loggingService.information(ids.matches.length)

      await Promise.all(ids.matches.map(async id => this.getMatch(id.uuid))).then(this.calculateStandings);
  ...
  async initTeam(teamId, cb){
    ...
    if (team && team.team) {
        this.dataService.updateTeam({"team.id": team.team.id},team,{upsert:true});
    } else {
  ...
  async updateTeamsAfterMatch(match){
    ...
    await this.dataService.updateTeam({_id:team1._id}, team1);
    await this.dataService.updateTeam({_id:team2._id}, team2);
  ...
  async getTeam(teamName){
    let team = await this.dataService.getTeam({"team.name":teamName.trim()});

    if (!team){
      const regex = new RegExp(teamName.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
      team = await this.dataService.getTeam({"team.name":{"$regex": regex}})
    }

    if (!team){
      team = await cyanideService.team({name:teamName});
      if (team && team.team){
        await this.dataService.updateTeam({"team.id": team.team.id},team,{upsert:true});
        return team;
      }
      else {
  ...
  async updateTeamPlayers(team){
    ...
    await Promise.all(team.roster.map(async function(currentPlayer){
      ...
      if (currentPlayer.id ===null){
        player = await this.dataService.getPlayer({teamId:team.idteamlisting , name: currentPlayer.name});
      } else{
        player = await this.dataService.getPlayer({id:currentPlayer.id});
      }
      ...
      if (!player) {
        ...
        this.dataService.updatePlayer({_id:player._id},player);
      }
    },this));

    //clear sustained casualties for players that weren"t in this match
    let activePlayers = await this.dataService.getPlayers({teamId:team.idteamlisting, active:true});
  ...
  async updateTeam(teamId){
    ...
    let currentTeam = await this.dataService.getTeam({"team.id":team.team.id});
    if (!currentTeam) {
    ...
    this.dataService.updatePlayers({"teamId":teamId},{active:false},{multi:true});

    let missingPlayers = 0;
    let missingTV = 0;
    await Promise.all(team.roster.map(async function(currentPlayer){
      
      let player = await this.dataService.getPlayer({id:currentPlayer.id});
      if(player && currentPlayer.id !== null){
        ...
        this.dataService.updatePlayer({_id:player._id},{"active":true, "skills":skills, "value":currentPlayer.value, "xp": currentPlayer.xp, "level":currentPlayer.level, "number":currentPlayer.number});
      } else {
        ...
        this.dataService.insertPlayer(currentPlayer);
      }
    ...
    this.dataService.updateTeam({_id:currentTeam._id},currentTeam);
  ...
  async updateCAS(team1, team2){
    ...
    const mng = ["BrokenJaw","BrokenRibs","FracturedArm","FracturedLeg","GougedEyes","GroinStrain","PinchedNerve","SmashedHand"];
    ...
    let league = {"name":"cripple", "stats":cas};

    let data = await this.dataService.getCasualties({"name":"cripple"});


    this.dataService.updateCasualties({"name":"cripple"},{$inc:cas} ,{upsert:true} )
  }

  async getCasualties(){
    return await this.dataService.getCasualties({"name":"cripple"});
  }
  ...
  async notifyClients(){
    ...
    this.wss.clients.forEach(s => s.readyState === s.OPEN ? s.send(JSON.stringify(cas)) : false);
  }
