...
class CrippleService{
  ...
  async getMatch(id){
    try {
      ...
      if (match.match.competitionname.indexOf("Cripple") > -1){
        match.match.competitionname = "REBBL Cripple Ladder - Season 5";
  ...
  async getMatches(){
    try {
      ...
      if (ids)
        await Promise.all(ids.matches.map(async id => this.getMatch(id.uuid))).then(data => this.calculateStandings("REBBL Cripple Ladder - Season 5",data));
