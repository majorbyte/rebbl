...
class CrippleService{
  ...
  async getMatches(){
    try {
      ...
      await Promise.all(ids.matches.map(async id => await this.getMatch(id.uuid))).then(async data => await this.calculateStandings("REBBL Cripple Ladder",data));
  ...
  async updateTeamsAfterMatch(match){
    ...
    try{
      ...
      this.updateCAS(matchTeam1, matchTeam2);
    } catch(ex){
      loggingService.error(ex);
    }
  ...
  async getCasualties(){
    return await dataService.getCasualties({"name":"cripple"});
  }
