...
class CrippleService{
  ...
  async getMatch(id){
    try {
      ...
      if (match.match.competitionname.indexOf("Cripple") > -1){
        ...
        dataService.insertMatch(match);
        if (match.match.competitionname === "REBBL's Christmas Cracker"){
          await this.updateTeamsAfterMatch(match);
        }
      }
  ...
  async getMatches(){
    try {
      ...
      if (ids)
        await Promise.all(ids.matches.map(async id => this.getMatch(id.uuid))).then(data => this.calculateStandings("REBBL Cripple Ladder - Season 4",data));
  ...
  async getStandings(){
    let standings = await dataService.getStandings({competition:/cripple/i});
