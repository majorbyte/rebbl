...
class CrippleService{
  ...
  async calculateStandings(competition, ids){
    ...
    let teamIds = []
    if(ids){
      let m = await this.dataService.getMatches({"uuid":{$in: ids}});

      const home = [...new Set(m.map(x => x.teams[0].id ))];
      const away = [...new Set(m.map(x => x.teams[1].id ))];
      teamIds = home.concat(away);

      matches = await this.dataService.getMatches({"match.competitionname":competition, "teams.id":{$in: teamIds}});


    } else {
      matches = await this.dataService.getMatches({"match.competitionname":competition});
    }
    ...
    result.map(r =>{
      this.dataService.updateStanding({"competition":r.competition, "teamId":r.teamId},r,{upsert:true});
    });
  }


  async getStandings(){
    let standings = await this.dataService.getStandings({});


    return await Promise.all(standings.map(async s => {
      let team = await this.dataService.getTeam({"team.id":s.teamId});
      if (team){ 
        s.team = team.team;
        s.coach = team.coach;
      }
