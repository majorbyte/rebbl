...
class ClanValidationService {
  ...
  async validateTeams(clan){
    ...
    for(let team of clan.ledger.teamBuilding){
      ...
      if (!test.test(team.name)) messages.push(`The team name ${team.name} does not match the format [${clan.name}] name S13`);
      ...
      if (t.team.cash > 0) messages.push(`${team.name}: needs to dump ${t.team.cash} after their first game.`);
