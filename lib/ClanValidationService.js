...
class ClanValidationService {
  constructor(){
    this.division1 = ['BBT','NOOB','FP','CLASSY','1PTTP1','COOKIE','BROMBE','REL','BBQ','SNAKES'];
    this.division2 = ['CO', 'ZENA', 'DUDE', 'SALTY', 'KLAANI', 'FABBL', 'JOKE', 'RISE', 'DUCK', 'HOME', 'METAL', 'DVLA', 'BOSC', 'READ', 'ANZAC', 'GODS', 'HUN', 'DUSTY', 'BBQ2', 'HUGE'];
  ...
  async validate(clanName){
    const clan = await dataService.getClan({name:clanName,season:'season 14'})
  ...
  checkFreshTeamBasicRules(teams){
    ...
    return teams.filter(x => !['Halfling','Goblin'].includes(x.race?.name)).map(team => team.fanfactor >= 5 ? '' : team.roster ? `Rule 1.1: ${team.name}, increase fan factor to at least 5` : '')
      .concat(teams.map(team => team.roster?.length >= 11 ? '' : team.roster ? `Rule 1.3: ${team.name} needs at least 11 players` : ''))
      .concat(teams.map(team => team.roster?.filter(x => !x.skill1 && !x.skill2).length >= 3 ? '' : team.roster ? `Rule 1.4: ${team.name} needs at least 3 rookie players with no skillups` : '' ))
      .filter(x => x !== '');
  }
  ...
  checkFreshTeamSuperstarsRules(teams){
    ...
    return teams.map(team => team.roster?.filter(player => player.skill1 && player.skill2).length <=5 ? '' : team.roster ? `Rule 3.1: ${team.name} has too many level 3 players` : '')
  ...
  async validateTeams(clan){
    ...
    for(let team of clan.ledger.teamBuilding){
      const messages = [];
      const test = new RegExp(`^\\[${clan.name}].*S13$`);
      if (!test.test(team.name)) {
        messages.push(`The team name ${team.name} does not match the format [${clan.name}] name S13`);
        errors.push({team:team.name || `team at position ${team.id }` ,messages});
        continue;
      }
      let t = await cyanide.team({name:team.name});
      if (!t) {
        messages.push(`Could not find team ${team.name} in BB2`);
        errors.push({team:team.name || `team at position ${team.id }` ,messages});
        continue;
      }
      if (t.team.cash > 0) messages.push(`Needs to dump ${t.team.cash} before their first game (buy & fire players, not rerolls/coaches/cheerleaeders)`);
      
      if (Number(t.team.rerolls) !== Number(team.rerolls)) messages.push(`Number of rerolls don't match (${t.team.rerolls} vs ${team.rerolls})`);
      if (Number(t.team.popularity) !== Number(team.fanfactor)) messages.push(`The fanfactor doesn't match (${t.team.popularity} vs ${team.fanfactor})`);
      if (Number(t.team.cheerleaders) !== Number(team.cheerleaders)) messages.push(`Number of cheerleaders don't match (${t.team.cheerleaders} vs ${team.cheerleaders})`);
      if (Number(t.team.assistantcoaches) !== Number(team.coaches)) messages.push(`Number of assistantcoaches don't match (${t.team.assistantcoaches} vs ${team.coaches})`);
      if (team.apothecary !== (t.team.apothecary == 1 ? true : false) ) messages.push(`Apothecary doesn't match`);
      if (Number(t.team.stadiumlevel) != 1) messages.push(`You can only have level 1 stadiums`);

      for(let player of team.roster||[]){
        const createdPlayer = t.roster?.find(p => p.name+"" === player.name+"");
        if (!createdPlayer) {
        ...
        if (player?.id > 0){
          ...
          if (player12.casualties_state.length !== player.casualties.length) messages.push(`Returning player ${player.name} number of casualties doesn't match S12 version`);
      ...
      for(let player of t.roster||[]){
        const ledgerPlayer = team.roster.find(p => p.name +"" === player.name +"");
        if (!ledgerPlayer) {
          messages.push(`Player ${player.name} can not be found on the ledger`);
          continue;
        }
      }
      if (messages.length > 0) errors.push({team:team.name || `team at position ${team.id }` ,messages});
    }
    return errors;
  }
