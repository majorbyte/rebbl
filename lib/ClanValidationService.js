...
class ClanValidationService {
  ...
  async validateTeams(clan){
    ...
    for(let team of clan.ledger.teamBuilding){
      ...
      if (Number(t.team.popularity) !== Number(team.fanfactor)) messages.push(`Number of cheerleaders don't match (${t.team.popularity} vs ${team.fanfactor})`);
      ...
      for(let player of team.roster||[]){
        ...
        if (!createdPlayer) {
          messages.push(`Could not find player ${player.name}`);
          continue;
        }
        if (createdPlayer.type !== player.playerType.type) messages.push(`Player ${$createdPlayer.name} does not match the position defined on the ledger`);
        ...
        if (player?.id > 0){
          ...
          if (player12.casualties_state.length < player12.casualties_sustained_total.length) player12.casualties_state = player12.casualties_sustained_total;
