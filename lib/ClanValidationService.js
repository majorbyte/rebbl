...
class ClanValidationService {
  ...
  checkFreshTeamBasicRules(teams){
    ...
    return teams.filter(x => !['Halfling','Goblin'].includes(x.race?.name))
      .map(team => team.fanfactor >= 5 ? '' : `Rule 1.1: ${team.name}, increase fan factor to at least 5`)
  ...
  async validateTeams(clan){
    ...
    for(let ledgerTeam of clan.ledger.teamBuilding){
      let messages = await this.compareTeams(clanName, ledgerTeam, ledgerTeam.name);
  ...
  async validateNewBlood(clanName,teamId, newTeam){

    let clan = await dataService.getClan({name:clanName, active:true});
    if (clan.powers.newBlood < 1) return "No more new blood left";

    let team = await dataService.getTeam({"team.id":teamId});
    if (!team) return "Current team not found";

    const now = Date.now();
    let lastMatchDate = new Date(team.team.datelastmatch);
    lastMatchDate.setHours(lastMatchDate.getHours() + 48);
    if (lastMatchDate < now) return "The 48 hour mark has passed";

    const ledgerTeam = clan.ledger.teams.find(x => x.id === teamId);
    if (ledgerTeam.value < 1100) return `The team ${ledgerTeam.name} is not valid for New Blood, its team value (${ledgerTeam.value}) is too low`;

    /* Check for draft */
    
    /* New Blood rules */
    
    //1:  replacing it with a new team costing the same in gold as the original team did at the start of the season
    const orignalTeam = clan.teamBuilding.find(x => x.name === team.team.name);
    const teamCost = this.teamCost(orignalTeam);
    const newTeamCost = this.teamCost(newTeam);
    ...
    messages =  messages.concat(this.compareTeams (clanName, newTeam, newTeam.name));
    return messages;
  }
