...
class ClanValidationService {
  ...
  async validateTeams(clan){
    ...
    for(let team of clan.ledger.teamBuilding){
      ...
      if (t.team.cash > 0) messages.push(`${team.name}: needs to dump ${t.team.cash} before their first game (buy & fire players, not rerolls/coaches/cheerleaeders)`);
      
      if (Number(t.team.rerolls) !== Number(team.rerolls)) messages.push(`${team.name}: number of rerolls don't match (${t.team.rerolls} vs ${team.rerolls})`);
      if (Number(t.team.popularity) !== Number(team.fanfactor)) messages.push(`${team.name}: number of cheerleaders don't match (${t.team.popularity} vs ${team.fanfactor})`);
      if (Number(t.team.cheerleaders) !== Number(team.cheerleaders)) messages.push(`${team.name}: number of cheerleaders don't match (${t.team.cheerleaders} vs ${team.cheerleaders})`);
      if (Number(t.team.assistantcoaches) !== Number(team.coaches)) messages.push(`${team.name}: number of assistantcoaches don't match (${t.team.assistantcoaches} vs ${team.coaches})`);
      if (team.apothecary !== (t.team.apothecary == 1 ? true : false) ) messages.push(`${team.name}: apothecary doesn't match`);
      if (Number(t.team.stadiumlevel) != 1) messages.push(`${team.name}: you can only have level 1 stadiums`);

      for(let player of team.roster){
        const createdPlayer = t.roster.find(p => p.name === player.name);
        if (!createdPlayer) {
          messages.push(`${team.name}: could not find player ${player.name}`);
          continue;
        }
        let skills = player.skills || [];
        if (player.skill1) skills.push(player.skill1);
        if (player.skill2) skills.push(player.skill2);
        const playerSkills = this.skills[player.playerType.type] || [];
        skills = skills.concat(playerSkills).map(x => x);
        for (let skill of skills){
          if (!createdPlayer.skills.map(skill => skill.replace(/ /g, '').trim()).includes(skill) && !playerSkills.includes(skill)) messages.push(`${team.name}: ${player.name} is missing skill ${skill}`);
        }
        for(let skill of createdPlayer.skills){
          if (!skills.includes(skill.replace(/ /g, '').trim())) messages.push(`${team.name}: ${player.name} has skill ${skill.replace(/([A-Z])/g, ' $1').trim()} too many`);
        }
      }
