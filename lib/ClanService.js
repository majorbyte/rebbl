...
class Clan {
  constructor(){
    this.division1 = ['BBQ','BBT','DVLA','REL','DUSTY','CO','READ','ANZAC','METAL','DUDE'];
    this.division2 = ['CLASSY','SALTY','KLAANI','GODS','BOSC','DOB','HUGE','KDOM','DREDD','W3W','SNAKES','HUN','FABBL','BBQ2','RISE','ZENA','JOKE','DCU','CTRM','VILE'];
  }
  async init(socket){
    this.socket = socket;
  }
  ...
  async getNewClanByUser(name) {
    let clan = await dataService.getClan({season:"season 15", "members.coach":new RegExp(`^${name}$`,'i')});
    if (!clan) clan = await dataService.getClan({season:"season 15", "leader":new RegExp(`^${name}$`,'i')});
    return clan
  }

  async getNewClanByName(name) {
    let clan = await dataService.getClan({season:"season 15", "name":new RegExp(`^${name}$`,'i')});
    return clan
  }
  ...
  async lockClan(clan){
    dataService.updateClan({name:clan, season:'season 15'},{$set:{locked:true}});
  }
  ...
  async skillPlayer(clanName, teamId, newPlayer ){
    ...
    const clan = await dataService.getClan({season:"season 15", name:clanName});
    ...
    dataService.updatePlayer({_id:player._id}, {$set:{diceRoll:{dice: [d1,d2], date:Date.now(), clan:clanName, season:'season 15'}}})
  ...
  async getContestData(){
    ...
    for(var x = 0; x < season.leagues.length; x++){
      ...
      for(let contest of contests.upcoming_matches){
        ...
        let s = {house:game[2], round:game[1], competition:`Division ${game[0]}`, season:"season 15"};
  ...
  async getCoachLastSeasonTeam(coach){
    let teams = await dataService.getTeams({'team.name':/^(?!.*proxy).*(S14)$/i, 'coach.name':new RegExp(`^${coach}$`,'i')});
  ...
  async calculateStandings(){
    let schedules = await dataService.getSchedules({league:"clan",season:"season 15"});

    const newScore = async function(clan,division){
      let c = await dataService.getClan({name:clan, season:"season 15"});
      return {
        league:"clan",
        division:division,
        season:"season 15",
    ...
    if (scores.length > 0){
      await dataService.removeStandings({league:"clan",season:"season 15"});
  ...
  async startCompetitions(username, division, round, house){
    ...
    const schedule = await dataService.getSchedule({league:"clan",season:"season 15", competition:division, round, house});
