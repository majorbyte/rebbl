...
async calculateStandings(){
  let schedules = await dataService.getSchedules({league:"clan",season:this.season});

  const newScore = async function(clan,division, season){
    let c = await dataService.getClan({name:clan, season});
    return {
      league:"clan",
      division:division,
      season:season,
  ...
  let scores = [];
    for(let schedule of schedules){
      ...
      if (!home) {
        home = await newScore(schedule.home.clan,schedule.competition, this.season);
        scores.push(home);
      } 
  
      let away = scores.find(x => x.clan.localeCompare( schedule.away.clan, undefined,{sensitivity:"base"}) ===0);
      if (!away) {
        away = await newScore(schedule.away.clan,schedule.competition, this.season);
        scores.push(away);
      } 
    ...
    if (scores.length > 0){
      await dataService.removeStandings({league:"clan",season:this.season});
