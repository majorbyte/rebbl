...
class Clan {
  ...
  async updateTeam(coach, teamId, newTeam){
    ...
    dataService.updateClan({_id:clan._id},clan);
  ...
  async skillPlayer(clanName, teamName, playerId ){
    const clan = await dataService.getClan({season:"season 13", name:clanName});
    if (!clan) throw new Error('Clan not found.');

    const team = clan.ledger.teamBuilding.find(x => x.name === teamName);
    if (!team) throw new Error('Team not found.');

    const player = await dataService.getPlayer({id: Number(playerId)});
