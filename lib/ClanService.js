...
class Clan {
  ...
  async getNewClanByName(name) {
    let clan = await dataService.getClan({season:"season 13", "name":new RegExp(`^${name}$`,'i')});
    return clan
  }
  ...
  async lockClan(clan){
    dataService.updateClan({name:clan, season:'season 13'},{$set:{locked:true}});
  }
  ...
  async getContestData(){
    ...
    for(var x = 0; x < season.leagues.length; x++){
      ...
      for(let contest of contests.upcoming_matches){
        ...
        let s = {house:game[2], round:game[1], competition:`Division ${game[0]}`, season:"season 13"};
  ...
  async calculateStandings(){
    let schedules = await dataService.getSchedules({league:"clan",season:"season 13"});

    const newScore = async function(clan,division){
      let c = await dataService.getClan({name:clan, season:"season 13"});
      return {
        league:"clan",
        division:division,
        season:"season 13",
    ...
    await dataService.removeStandings({league:"clan",season:"season 13"});
  ...
  async startCompetitions(username, division, round, house){
    ...
    const schedule = await dataService.getSchedule({league:"clan",season:"season 13", competition:division, round, house});
