...
const { league, competitions } = require("./CyanideService.js");
...
class Clan {
  ...
  async getContestData(){
    ...
    for(var x = 0; x < season.leagues.length; x++){
      ...
      for(let contest of contests.upcoming_matches){
        ...
        await dataService.updateScheduleAsync(s,{$set:{matches:schedule.matches,"away.score":schedule.away.score,"home.score":schedule.home.score, unstarted:schedule.unstarted}});
      }
    }
  ...
  async calculateStandings(){
    ...
    for(let schedule of schedules){
      ...
      if (schedule.matches.length === 0) continue;
  ...
  async startCompetitions(username, division, round, house){
    ...
    const schedule = await dataService.getSchedule({league:"clan",season:"season 9", competition:division, round, house});
