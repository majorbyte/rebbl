...
class Clan {
  ...
  async getNewClanByUser(name) {
    let clan = await dataService.getClan({season:"season 14", "members.coach":new RegExp(`^${name}$`,'i')});
    if (!clan) clan = await dataService.getClan({season:"season 14", "leader":new RegExp(`^${name}$`,'i')});
    return clan
  }

  async getNewClanByName(name) {
    let clan = await dataService.getClan({season:"season 14", "name":new RegExp(`^${name}$`,'i')});
    return clan
  }
  ...
  async lockClan(clan){
    dataService.updateClan({name:clan, season:'season 14'},{$set:{locked:true}});
  }
  ...
  async updateClan(coach, clan){
    ...
    dataService.updateClan({_id:c._id},clan);
  ...
  async skillPlayer(clanName, teamId, newPlayer ){
    ...
    const clan = await dataService.getClan({season:"season 14", name:clanName});
    ...
    dataService.updatePlayer({_id:player._id}, {$set:{diceRoll:{dice: [d1,d2], date:Date.now(), clan:clanName, season:'season 14'}}})
  ...
  async getContestData(){
    ...
    for(var x = 0; x < season.leagues.length; x++){
      ...
      for(let contest of contests.upcoming_matches){
        ...
        let s = {house:game[2], round:game[1], competition:`Division ${game[0]}`, season:"season 14"};
  ...
  async getCoachLastSeasonTeam(coach){
    let teams = await dataService.getTeams({'team.name':/^(?!.*proxy).*(S13)$/i, 'coach.name':new RegExp(`^${coach}$`,'i')});

    return teams.sort((a,b) => b.team.date_last_match  > a.team.date_last_match ? -1 : 1 )[0];
  }
  ...
  async calculateStandings(){
    let schedules = await dataService.getSchedules({league:"clan",season:"season 14"});

    const newScore = async function(clan,division){
      let c = await dataService.getClan({name:clan, season:"season 14"});
      return {
        league:"clan",
        division:division,
        season:"season 14",
    ...
    if (scores.length > 0){
      await dataService.removeStandings({league:"clan",season:"season 14"});
  ...
  async startCompetitions(username, division, round, house){
    ...
    const schedule = await dataService.getSchedule({league:"clan",season:"season 14", competition:division, round, house});
