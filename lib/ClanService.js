...
class Clan {
  ...
  async getContestData(clean){
    ...
    */
    ...
    let competitions = data.responseGetCompetitions.competitions.competition.filter(x => x.status > 1).map(x => x.id);
    
    jsonResponse = await fetch(`${this.apiUrl}/api/league/${this.leagueId}/competitions/50/50`);

    data = await jsonResponse.json(); 
    competitions = competitions.concat(data.responseGetCompetitions.competitions.competition.filter(x => x.status > 1).map(x => x.id));
    
    
    for(let id of competitions){
    ...
    const unstarted = [];
    for(const competition of data.competitions.filter(x => x.status == 1)){
      let game = competition.name.match(/[0-9]+/g).map(Number);
      if (!unstarted.some(x => x.house == game[2] && x.round == game[1])) unstarted.push({house:game[2], round:game[1]});
    }

    for(const item of unstarted) await this.#updateUnstarted("Division 1", item.round, item.house);
...
async calculateStandings(season){
  ...
  async getCompetitionInformation(division, round, house){
    ...
    let competitions = await this.#getCompetitionInformation(division, house,round);
  ...
  async #updateUnstarted(division, round, house){
    ...
    let competitionsInformation = await this.getCompetitionInformation(division, round, house);
  ...
  async #getCompetitionInformation(division, house, round){
