...
class Clan {
  ...
  async getNewClanByUser(name) {
    let clan = await dataService.getClan({season:"season 13", "members.coach":new RegExp(`^${name}$`,'i')});
    if (!clan) clan = await dataService.getClan({season:"season 13", "leader":new RegExp(`^${name}$`,'i')});
    return clan
  }
  ...
  async _correctDicerolls(team){
    ...
    for(let player of team.roster.filter(player => player.dice)) {
      const playerS12 = await dataService.getPlayer({id:player.id});
      
      if (playerS12.level !== player.level) continue;

      const nextSPP = SPP.find(x => x.level === playerS12.level+1);
      const requiredSPP = nextSPP.spp - playerS12.xp;
  ...
  async updateClan(coach, clan){
    const c = await this.getNewClanByUser(coach);
    for(let team of clan.ledger.teamBuilding){
      await this._correctDicerolls(team);
    }
    await dataService.updateClan({_id:c._id},clan);
    this.socket.in(clan.name).emit('clan',JSON.stringify(clan));
  }

  async updateTeam(coach, teamId, newTeam, emit){
    const clan = await this.getNewClanByUser(coach);
    ...
    await dataService.updateClan({_id:clan._id},clan,{upsert:true,writeconcern:{w:1}});

    if (emit) this.socket.in(clan.name).emit('team',JSON.stringify(newTeam));

  }
  async updateMembers(coach, members){
    const clan = await this.getNewClanByUser(coach);

    await dataService.updateClan({_id:clan._id},{$set:{members:members}});
  }
  ...
  async skillPlayer(clanName, teamId, newPlayer ){
    ...
    const clan = await dataService.getClan({season:"season 13", name:clanName});
    ...
    await dataService.updateClan({_id:clan._id},clan,{upsert:true});
    dataService.updatePlayer({_id:player._id}, {$set:{diceRoll:{dice: [d1,d2], date:Date.now(), clan:clanName}}})

    return clan.ledger.teamBuilding[index].roster[playerIndex];
  }

  async setLogo(name, file){
    await dataService.updateClan({name:name, active:true},{$set:{logo:file}});
  }
  ...
  async getCoachLastSeasonTeam(coach){
    let teams = await dataService.getTeams({'team.name':/s12/i, 'coach.name':new RegExp(`^${coach}$`,'i')});

    return teams.sort((a,b) => b.team.id - a.team.id)[0];
  ...
  async newBlood(clanName,teamId,newTeamName){
    ...
    await dataService.updateClan({name:clanName, active:true},{$push:{"ledger.teams":newTeam},$set:{teams:clan.teams}});
    
    await dataService.updateClan({name:clanName, active:true, "ledger.teams.team.id":teamId},{$set:{"ledger.teams.$.active":false}});
  ...
  async newCoach(clanName,teamId,newTeamName){
    ...
    await dataService.updateClan({name:clanName, active:true},{$push:{"ledger.teams":newTeam},$set:{teams:clan.teams, members:clan.members}});
