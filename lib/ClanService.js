...
class Clan {
  ...
  async getContestData(){
    ...
    let options = {platform:"pc", league : "05004f31-9d5a-11ef-aaf1-bc24112ec32e", contest_status:"*", limit:70};
    let data = await cyanideService.contests(options);
    requestCount++;
    ...
    for(let contest of data.contests){
      ...
      if (contest.game_id !== null && !c.counted){
        c.counted = true;
...
async calculateStandings(season){
  ...
  async updateUnstarted(division, round, house){
    ...
    else for(let x = 1; x < 13; x++) await this.#updateUnstarted(division,1,house);
  }
