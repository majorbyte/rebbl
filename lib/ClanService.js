...
class Clan {
  ...
  async getContestData(){
    ...
    for(var x = 0; x < season.leagues.length; x++){
      ...
      for(let contest of contests.upcoming_matches){
        ...
        let s = {house:game[2], round:game[1], competition:`Division ${game[0]}`, season:"season 10"};
        ...
        if(!c) {
          ...
          schedule.unstarted.splice(schedule.unstarted.findIndex(x => x.competitionId === contest.competition_id),1);
  ...
  async calculateStandings(){
    let schedules = await dataService.getSchedules({league:"clan",season:"season 10"});

    const newScore = async function(clan,division){
      let c = await dataService.getClan({name:clan, season:"season 10"});
      return {
        league:"clan",
        division:division,
        season:"season 10",
    ...
    await dataService.removeStandings({league:"clan",season:"season 10"});
  ...
  async startCompetitions(username, division, round, house){
    ...
    const schedule = await dataService.getSchedule({league:"clan",season:"season 10", competition:division, round, house});
    ...
    const isMember = members.find(x => x.reddit === username);
