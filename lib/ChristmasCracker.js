...
Violence Together teams, who may additionally hire one Goblin Troll or one Ogre Ogre in their initial roster. MercenarySavage
  ...
  async claimPacks(coachName){
    ...
    coach = coach[0];
    //Nice
    let nice = coach.touchdownsPacks + coach.completionsPacks + coach.levelsPacks + coach.armourBreaksPacks;

    //Naughty
    let naughty = coach.casualtiesPacks + coach.killsPacks + coach.surfsPacks + coach.weaponsPacks;

    //Filler
    let filler = coach.matchesPlayedPacks + coach.streakPacks;
    ...
    let r = await axios.delete(url,options);
    ...
    await createPacks("stocking filler",filler);
