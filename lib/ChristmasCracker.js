...
, crippleService = require("./DataService.js").cripple;
...
class ChristmasCracker {
  constructor(){
    ...
    this.races = [
      {id:32,name:"MercenaryAristo"},
      {id:33,name:"MercenaryChaos"},
      {id:34,name:"MercenaryChaosGods"},
      {id:35,name:"MercenaryEasterners"},
      {id:36,name:"MercenaryElf"},
      {id:37,name:"MercenaryExplorers"},
      {id:38,name:"MercenaryGoodGuys"},
      {id:39,name:"MercenaryHuman"},
      {id:40,name:"MercenarySavage"},
      {id:41,name:"MercenaryStunty"},
      {id:42,name:"MercenaryUndead"}
    ];
  ...
  async registerTeam(coach, teamName){
    ...
    crippleService.insertStandings([standing]);
    ...
    crippleService.updateTeam({"team.id": team.team.id},team,{upsert:true});
  ...
  async _checkTeam(coachName,team){
    ...
    let coach = await crippleService.getStandings({league:/rebbl off season/i, competition:/REBBL's Christmas Cracker/i, coach:{$regex:coachEx}});
    ...
    if (coach.length === 0){
      ...
      const allowed = (value) => this.startingTypes.includes(value);
      let errors =[];
      ...
      checkTypes.map(t => {
        ...
        const expectedLength = ["MercenaryUndead_Undead_Skeleton", "MercenaryUndead_Undead_Zombie"].includes(t) ? 4 : 2;
  ...
  async checkAchievements(){
    ...
    if (this.config == null){
      this.config = await crippleService.getConfiguration({name:"crackerConfig"});
    }

    let coaches = await crippleService.getStandings({league:/rebbl off season/i, competition:/REBBL's Christmas Cracker/i});

    //let matches = await crippleService.getMatches({"match.leaguename":/rebbl off season/i, "match.competitionname":/REBBL's Christmas Cracker/i});
    let matches = await crippleService.getMatches({"match.leaguename":/Imperium/i, "match.competitionname":/Gimel 91 Boot/i});


    coaches.map(x => this._resetPoints(x));

    coaches.map(coach => {
      let coachMatches = matches.filter(match => match.match.teams[0].idteamlisting === coach.teamId || match.match.teams[1].idteamlisting === coach.teamId );
    ...
    await crippleService.removeStandings({league:/rebbl off season/i, competition:/REBBL's Christmas Cracker/i});
    await crippleService.insertStandings(coaches);
  ...
  _calculatePacks(coach){
    ...
    coach.killsPacks = coach.kills >= this.config.init.kills ? 1 + Math.floor((coach.kills-this.config.init.kills) / this.config.kills) : 0; 
    coach.completionsPacks = coach.completions >= this.config.init.completions ? 1 + Math.floor((coach.completions-this.config.init.completions) / this.config.completions) : 0; 
    ...
    coach.matchesLostPacks = coach.matchesLost >= this.config.init.matchesLost ? 1 + Math.floor((coach.matchesLost-this.config.init.matchesLost) / this.config.step) : 0; 
  }
