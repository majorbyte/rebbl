...
class LeagueService{
  ...
  async getUpcomingMatch(_redditUser, user=String(_redditUser)){
    ...
    let coach = await accountService.searchAccount({"coach": match.opponents[0].coach.name});
    match.opponents[0].coach = Object.assign(match.opponents[0].coach, coach);

    coach = await accountService.searchAccount({"coach": match.opponents[1].coach.name});
