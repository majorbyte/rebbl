...
class LeagueService{
  ...
  originalFind(filter){
    return this.schedule.origFind(filter);
  }
  ...
  async getCoachScore(league, competition, group) {
    ...
    schedules.forEach(function (schedule) {
      let isConfigured = seasons.find(function(season){ return season.leagues.find(function(league){ 
        ...
        return (league.name.toLowerCase() === schedule.league.toLowerCase()  && (league.divisions.indexOf(schedule.competition) > -1)) 
        ||  (league.name.toLowerCase() === schedule.league.toLowerCase()  && isRampup) 
        || (league.name.toLowerCase() === schedule.league.toLowerCase()  && league.multi)
        })
      });
    ...
    if (group){
      ...
      for(let div in data){
        ...
        for(let index in data[div]){
          ...
          let matches = schedules.filter(s => s.competition = coach.competition && s.opponents && (s.opponents[0].coach.id === coach.id ||s.opponents[1].coach.id === coach.id) );
  ...
  async getStuntyStandings(){
    ...
    let regex = new RegExp(`^Season 10`, "i");
  ...
  async LinemanleagueFix(){

    this.schedule.update({"competition_id":101226},{$set:{"competition":"Season 4 - Alpha"}},{multi:true});
    this.schedule.update({"competition_id":101227},{$set:{"competition":"Season 4 - Bravo"}},{multi:true});
    this.schedule.update({"competition_id":101228},{$set:{"competition":"Season 4 - Charlie"}},{multi:true});
    this.schedule.update({"competition_id":101230},{$set:{"competition":"Season 4 - Delta"}},{multi:true});
    this.schedule.update({"competition_id":101231},{$set:{"competition":"Season 4 - Echo"}},{multi:true});
