...
class LeagueService{
  ...
  async getStuntyStandings(){
    ...
    let schedules = await dataService.getSchedules({league:/^rebbl|rampup/i, "opponents.team.race" : {$in :stunties}, "season": "season 25"});
  ...
  async getUnplayedMatch(_contestId){
    //normal

    const contestId = isNaN(_contestId) ? _contestId : Number(_contestId);
    ...
    if (match.length > 0) {
      match = match[0].matches[0];
      let coach = await dataService.getAccount({bb3id: match.opponents[0].coach.id});

      if (coach){
        match.opponents[0].coach.reddit = coach.reddit;
        match.opponents[0].coach.discord = coach.discord;
        match.opponents[0].coach.timezone = coach.timezone;
      }
      
      coach = await dataService.getAccount({bb3id: match.opponents[1].coach.id});
      if (coach){
        match.opponents[1].coach.reddit = coach.reddit;
        match.opponents[1].coach.discord = coach.discord;
        match.opponents[1].coach.timezone = coach.timezone;
      }
