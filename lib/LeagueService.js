...
class LeagueService{
  ...
  async getStuntyStandings(){
    ...
    let regex = new RegExp(`^Season 12`, "i");
  ...
  async _unplayedMatch(match){
    ...
    if(match.opponents[0].coach.name){
      let regex = new RegExp(`^${match.opponents[0].coach.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i");
      ...
      match.opponents[0].coach.name="DO NOT PLAY THE AI";
      match.opponents[0].coach.reddit="AI";
      match.opponents[0].coach.id=0;
    }

    let team = await teamService.getTeamById(match.opponents[0].team.id);
    match.opponents[0].team =  Object.assign(match.opponents[0].team, team);

    if(match.opponents[1].coach.name){
      let regex = new RegExp(`^${match.opponents[1].coach.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i");
      let coach = await accountService.searchAccount({"coach":  {"$regex": regex}});
      match.opponents[1].coach = Object.assign(match.opponents[1].coach, coach);
    } else{
      match.opponents[1].coach.name="DO NOT PLAY THE AI";
      match.opponents[1].coach.reddit="AI";
      match.opponents[1].coach.id=0;
    }

    team = await teamService.getTeamById(match.opponents[1].team.id);
  ...
  async getUpcomingMatch(_redditUser, user=String(_redditUser)){
    ...
    let schedules = await dataService.getSchedules({"opponents.coach.name": {"$regex": regex}, "status":"scheduled"} );
    ...
    let clanSchedules = await dataService.getSchedules( {"matches":{$elemMatch:{"opponents.coach.name":  {"$regex": regex}, status:"scheduled" , match_uuid:null}}},{projection:{"matches.$":1}} );
