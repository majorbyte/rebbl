...
class LeagueService{
  ...
  async getMatchDetails(_id, matchId=String(_id)){
    ...
    if (!match){
      noTeamLink = true;
      match = await crippleService.getMatch({uuid:matchId});
    }
    ...
    return {"match": match.match, rosterSize:rosterSize,noTeamLink:noTeamLink};
  }
  ...
  async getStuntyStandings(){
    ...
    let schedules = await dataService.getSchedules({league:/^rebbl|rampup/i, "opponents.team.race" : {$in :stunties}, "season": "season 15"});
