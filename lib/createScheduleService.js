...
class CreateScheduleService{
  ...
  async create(competitionId){
    ...
    response = await fetch(`${this.apiUrl}/api/competition/${settingId}/participants/${participants}`,{
      method: "PUT",
      headers:{"Content-Type":"application/json"}
    });

    response = await fetch(`${this.apiUrl}/api/competition/${settingId}/timer/${timer}`,{
      method: "PUT",
      headers:{"Content-Type":"application/json"}
    });

    response = await fetch(`${this.apiUrl}/api/competition/${settingId}/password/%20`,{
      method: "PUT",
      headers:{"Content-Type":"application/json"}
    });

    response = await fetch(`${this.apiUrl}/api/competition/${settingId}/admissionMode/${3}`,{
      method: "PUT",
      headers:{"Content-Type":"application/json"}
    });

    response = await fetch(`${this.apiUrl}/api/competition/${settingId}/contestFormat/${1}`,{
      method: "PUT",
      headers:{"Content-Type":"application/json"}
    });
