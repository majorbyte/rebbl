...
const colorHelper = require('./colorhelper.js');

class StandingsService{
    newScore(league, comp,competitionId, season, opponent, points, tdDiff){
      return {
    ...
    async updateStandings(season, league, competition){
      [/REBBL - GMan 2/i,/REBBL - Rel 2/i].map(x => {
        if(x.test(league)) league = league.replace(" 2","");
      });
      
      const result = await this._calculateStandings(season, league,competition);

      if (result.length > 0){
        await dataService.removeStandings({"season": season, "league": new RegExp(league.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),"i"), "competition":new RegExp(`^${competition}$`,"i")});
        await dataService.insertStandings(result);
      }
    }
    async _calculateStandings(season, league, competition){
      let initMatches = await dataService.getSchedules({"season": season,"league": new RegExp(league.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),"i"), "competition":new RegExp(`^${competition}$`,"i"), "match_uuid":null, round:1});

      let matches = await dataService.getSchedules({"season": season,"league": new RegExp(league.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),"i"), "competition":new RegExp(`^${competition}$`,"i"), "match_uuid":{$ne:null}});
      let coaches = [];
      ...
      const groupedMatches = this._groupBy(matches, "round")
      ...
      for (const round in groupedMatches){
        groupedMatches[round].forEach(match => {
          if (match.winner){
              let winner = coaches.find(function (c) {
                ...
                let loserCoach = coaches.find(function (c) {
                  return c.id === (match.winner.coach.id === match.opponents[0].coach.id ? match.opponents[1].coach.id : match.opponents[0].coach.id);
              });
              
              let loser = match.winner.coach.id === match.opponents[0].coach.id ? match.opponents[1] : match.opponents[0];

              //process winner
              if(!winner){
                  winner = this.newScore(league,competition,match.competition_id, match.season, match.winner,3,match.winner.team.score - loser.team.score); 
                  coaches.push(winner);
              } else {
                  winner.games++;
                  winner.win++;
              ...
              //process loser
              if(!loserCoach){
                  loserCoach = this.newScore(league,competition,match.competition_id,match.season,loser,0,loser.team.score-match.winner.team.score); 
              ...
              if (!coach) {
                  ...
                  coach = this.newScore(league,competition,match.competition_id,match.season,match.opponents[0],1,0); 
                  coaches.push(coach);
              } else {
                  coach.games++;
                  coach.points++;
              ...
              if (!coach) {
                  coach = this.newScore(league,competition,match.competition_id,match.season,match.opponents[1],1,0); 
                  coaches.push(coach);
              } else {
                  coach.games++;
                  coach.points++;
        ...
        coaches.map((coach, index) => coach.progression.push(coach.position));
      }
      ...
      coaches.map((coach,index) => coach.position = index+1);
      coaches.map(coach => coach.logoColor = colorHelper.rgb(coach.logo));
