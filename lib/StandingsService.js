...
class StandingsService{
    ...
    async updateStandings(league, competition){
      ...
      if (result.length > 0){
        await dataService.removeStandings({"league": new RegExp(league.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),"i"), "competition":new RegExp(`^${competition}$`,"i")});
        await dataService.insertStandings(result);
      }
    }
    async _calculateStandings(league, competition){
      let initMatches = await dataService.getSchedules({"league": new RegExp(league.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),"i"), "competition":new RegExp(`^${competition}$`,"i"), "match_uuid":null, round:1});

      let matches = await dataService.getSchedules({"league": new RegExp(league.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),"i"), "competition":new RegExp(`^${competition}$`,"i"), "match_uuid":{$ne:null}});
      let coaches = [];
