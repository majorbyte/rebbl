...
class PerpetualService{
  ...
  async getMatch(id){
    try {
      ...
      match.match.competitionname = "ReBBL Perpetual Ladder - Season 1.2";
  ...
  async getMatches(){
    try {
      ...
      const ids = await cyanideService.matches({league:"REBBL Off Season", competition:"ReBBL Perpetual Ladder", start: now.toISOString(),id_only:1,limit:2000});

      if (ids)
        await Promise.all(ids.matches.map(async id => this.getMatch(id.uuid)));

        await this.calculateStandings("ReBBL Perpetual Ladder - Season 1.2");
  ...
  async calculateStandings(competition){
    ...
    let result = await Promise.all(teamIds.map(async team => {
      ...
      let index = m[0].match.teams[0].idteamlisting === team ? 0 : 1;

      const teamName = m[0].match.teams[index].teamname;
      const coach = m[0].match.coaches[index].coachname;
      const race = m[0].match.teams[index].idraces;

      let t = await dataService.getTeam({"team.id":team});
      const teamExists = t ? true: false;
      return {teamId:team,teamExists:teamExists, score, gp:gamesPlayed, wins, draws, limFactor, winPercentage, competition, season:"season 1", league:"rebbl perpetual",team:teamName, coach, race};
    }));
...
module.exports = new PerpetualService();
