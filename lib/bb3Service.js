...
class BB3Service{
  ...
  async updateSchedules (competition, day){
    ...
    for(const schedule of schedules){
      ...
      if(!scheduleExists || existingSchedule.status != schedule.status || existingSchedule.round != schedule.round || homeTVChanged || awayTVChanged || teamChangedButNotPlayed ){
    ...
    await dataService.deleteSchedules({competitionId:competition.id, round:Number(day), status:0});
  ...
  async updateCompetitions (leagueId){
    try{
      ...
      const parentCompetitions = [
        {id:"00000004-0004-0002-0000-000000000000", name:"BigO Premier Division", season:"season 4"},
        {id:"00000004-0004-0003-0000-000000000000", name:"Big O Fresh Division", season:"season 4"},
        {id:"00000000-0004-0000-0000-000000000000", name:"GMAN Premier Division", season:"season 4"},
        {id:"00000000-0004-0001-0000-000000000000", name:"GMAN Fresh Division", season:"season 4"},
        {id:"00000004-0004-0004-0000-000000000000", name:"REL Premier Division", season:"season 4"},
        {id:"00000004-0004-0005-0000-000000000000", name:"REL Fresh Division", season:"season 4"},
      ];
      ...
      for(let competition of competitions){
        ...
        let parentCompetition = parentCompetitions.find(x => competition.name.startsWith(x.name));
        if (parentCompetition){
          competition.parentId = parentCompetition.id;
          competition.season = parentCompetition.season;
        }
  ...
  async checkClosedCompetitions(leagueId){

    const competitions = await dataService.getCompetitions({season:"season 4", leagueId});

    for(const competition of competitions){
      if (competition.isParent) continue;

      let response = await fetch(`${this.apiUrl}/api/competition/${competition.id}`);
      if (!response.ok) continue;
      
      let result = (await response.json()).responseGetCompetition.competition;

      const status = Number(result.status);
      await dataService.updateCompetition({id:competition.id},{$set:{status}});
    }
  }
  ...
  async #updateTeam(teamId, logos,zfl){
    ...
    if (team.hasRecrutableJourneymen === "1") {
      response = await fetch(`${this.apiUrl}/api/team/${team.id}/recruitable`);
      if (!response.ok) team.journeymen = [];
      else {
        let players = (await response.json()).responseGetTeamRecruitableJourneymen.journeymen.player;
        team.journeymen = Array.isArray(players) ? players : [players];
      }
    } else {
      team.journeymen = [];
    }
  ...
  async calculateStandings(competitionId){
    ...
    this.calculateStrengthOfSchedule(coaches,matches);
    ...
    coaches = coaches.sort(
      function(a,b){
        ...
        //tie-breaker #2 : strength of schedule
        if (a.strengthOfSchedule > b.strengthOfSchedule) return -1;
        if (a.strengthOfSchedule < b.strengthOfSchedule) return 1;

        //tie-breaker #3 : Loss diff
  ...
  calculateStrengthOfSchedule(standings, schedules){

    //calculate strength
    standings.forEach(c => c.strength = c.points/((c.win + c.draw + (c.loss > 1 ? 2 : c.loss))*3));
    standings.filter(c => isNaN(c.strength)).forEach(c => c.strength = 0);

    //calculate strength of schedule
    for(let index in standings){
      let coach = standings[index];
      let strengthOfSchedule = 0;

      let matches = schedules.filter(s => s.home.coach.id === coach.id ||s.away.coach.id === coach.id);

      for (let index = 0; index < matches.length; index++) {
        let match = matches[index];

        const homeId = match.home.coach.id;

        const awayId = match.away.coach.id;

        let opponentCoach = homeId === coach.id
          ? standings.find(n => n.id === awayId)
          : standings.find(n => n.id === homeId);
