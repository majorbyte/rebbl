...
class BB3Service{
  ...
  async updateSchedules (competition){
    ...
    const mapSchedule = schedule => {
      return {
        ...
        gameId: schedule.matches.match.gameId,
  ...
  async updateCompetitions (leagueId){
    try{
      ...
      for(const competition of competitions){
        ...
        if (competition.status >= 3)await this.updateMatches(competition.id, updatedSchedules);
      }
    }
    catch(e){
  ...
  async updateMatches (competitionId,schedules){
    ...
    const matches = schedules.map(x => x.gameId);
    for(const matchId of matches){
      ...
      const schedule = schedules.find(x => x.matchId == matchId);
...
*/
  ...
  async processMatch(matchId, user, valid){
    const match = await dataService.getMatch({matchId});
    ...
    if (valid) await this.#validateMatch(match,user);
    else await this.#invalidateMatch(match,user);

    if (match.validatedBy && match.validatedBy.length > 1 ){
      await axios.post(`${this.apiUrl}/api/competition/validate/${match.matchId}/${match.gameId}`);
  ...
  async #downloadReplay(schedule){
    ...
    response = await axios.get(`${this.apiUrl}/api/cabaltv/${match.gameId}/download`); 
    if (response.data.replayData === "") return;

    await fs.writeFile(`./replays/bb3/${match.gameId}.bbrz`, response.data.responseDownloadReplay.replayData);
    await dataService.updateMatch({matchId:schedule.matchId},{$set:{replayFile:`${match.gameId}.bbrz`}});
  }
