...
class BB3Service{
  ...
  updatePlayerStatistics = async function(playerIds, teamId){
    const stats = [];
    for(const matchPlayer of playerIds){
      const response = await axios.get(`${this.apiUrl}/api/statistics/player/${matchPlayer.id}`);
      
      const current_statistics = response.data.responseGetPlayerStatistics.playerStatistics.statistics.statistic.map(this._mapStatistic);
      let player = await dataService.getPlayer({id:matchPlayer.id});

      if (!player) {
        player = object.assign(matchPlayer);
  ...
  calculateLadderStandings = async function(competitionId){
  ...
  calculateStandings = async function(competitionId){
    ...
    matches.forEach(match => {
      if (match.home.score == match.away.score){
        ...
        if (!coach) {
          coach = this._newScore(match.home,1,0); 
          coaches.push(coach);
        } else {
          coach.games++;
          coach.points++;
        ...
        if (!coach) {
          coach = this._newScore(match.away,1,0); 
          coaches.push(coach);
        } else {
          coach.games++;
          coach.points++;
        ...
        //process winner
        if(!winner){
            winner = this._newScore(winningTeam,3,winningTeam.score - losingTeam.score); 
            coaches.push(winner);
        } else {
            winner.games++;
            winner.win++;
        ...
        //process loser
        if(!loser){
          loser = this._newScore(losingTeam,0,losingTeam.score-winningTeam.score); 
          coaches.push(loser);
        } else {
          loser.games++;
          loser.loss++;
  ...
  _newScore = function(opponent, points, tdDiff){
    return {
        id: opponent.coach.id,
        name: opponent.coach.name,
        team: opponent.team.name,
  ...
  _mapStatistic = function (stat){
    ...
    if (stat.value.indexOf("/")>-1){
      const val = stat.value.split("/");
      s.value = Number(val[0]);
      s.total = Number(val[1]);
    } else {
      s.value = Number(stat.value);
    }
    return s;
  }
