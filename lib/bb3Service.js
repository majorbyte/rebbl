...
class BB3Service{
  ...
  async updateMatches (competitionId){
    ...
    for(const matchId of matches){
      let response  = await axios.get(`${this.apiUrl}/api/game/${matchId}`);
      const match = response.data.responseGetGameResult.gameResult;
  
      response = await axios.get(`${this.apiUrl}/api/statistics/match/${matchId}`);
      match.statistics = response.data.responseGetMatchStatistics.matchStatistics;
  
      //response = await axios.get(`${this.apiUrl}/api/game/${matchId}/spp`);
      //match.spp = response.data.responseGetSppResult.sppResult;
  
  
      response = await cyanideService.match({bb:3, match_id:match.gameId});

      match.homeTeam.logo.icon = response.match.teams[0].teamlogo + '.png';//await this.#getLogo(match.homeTeam.logo.itemId);
      match.awayTeam.logo.icon = response.match.teams[1].teamlogo + '.png';//await this.#getLogo(match.awayTeam.logo.itemId);
  ...
  async getUpcomingMatch(_redditUser,_coachId, user=String(_redditUser), coachId=String(_coachId)){
    ...
    for(const schedule of schedules){
      if(schedule.home.coach.id === account.bb3id ){
        if(/^\[admin].+/i.test(schedule.home.team.name)){
          schedules.splice(i,1);
          continue;
        }
      }
  
      if (schedule.away.coach.id === account.bb3id){
        if(/^\[admin].+/i.test(schedule.away.team.name)){
          schedules.splice(i,1);
        }
      }
    }
  ...
  async getUnplayedMatch(contestId){
    //normal
    let match = await dataService.getSchedule({id: contestId});
    if (match) return [await this.#unplayedMatch(match)];    
    return [];
  }

  async validateMatch(matchId, user){
    const match = await dataService.getMatch({matchId});
    ...
    if (match.validatedBy.length > 1 || (isAdmin && !isCoach)){
      // (isAdmin && !isCoach) -> don't automatically admin-validate your own games
      let response = await axios.post(`${this.apiUrl}/api/competition/validate/${match.matchId}/${match.gameId}`);
