...
class BB3Service{
  ...
  async updateSchedules (competition, day){
    ...
    for(const schedule of schedules){
      ...
      const teamChanged = existingSchedule.home.team.id != schedule.home.team.id || existingSchedule.away.team.id != schedule.away.team.id;
      const teamChangedButNotPlayed = teamChanged && schedule.status < 3;
      const homeTVChanged = existingSchedule.home.team.valueWithJourneymen != schedule.home.team.valueWithJourneymen;
      const awayTVChanged = existingSchedule.away.team.valueWithJourneymen != schedule.away.team.valueWithJourneymen; 
      
      if(!existingSchedule || existingSchedule.status != schedule.status || homeTVChanged || awayTVChanged || teamChangedButNotPlayed){
