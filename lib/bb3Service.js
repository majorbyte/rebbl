...
class BB3Service{
  ...
  async updateSchedules (competition, day){
    let response = await axios.get(`${this.apiUrl}/api/competition/schedule/${competition.id}/${day ?? competition.day}`);
    ...
    for(const schedule of schedules){
      ...
      if(!existingSchedule || existingSchedule.status != schedule.status 
        ...
        if (existingSchedule.gameId !== null && schedule.gameId === null) schedule.gameId == existingSchedule.gameId; 
...
*/
  ...
  async checkRetiredPlayers(){
    const retiredPlayers = await dataService.getRetiredPlayers({});
    ...
    for(const teamId of teamIds){
      try{
        ...
        const cookie = await adminBB3Service.setTeamToAdmin(null, competition.id, teamId);
  ...
  async retirePlayer(user, teamId, playerId){
    ...
    if (!Array.isArray(player.player.casualties)) player.player.casualties = [player.player.casualties];
    
    if (!player.player.casualties.some(x => Number(x.casualtiesItem) >= 5 )) return false;

    const retiredPlayer = player.player;
    retiredPlayer.teamId =  teamId;

    await dataService.insertRetiredPlayer(retiredPlayer);
  ...
  async handleRetiredPlayers(competitionId, teamId){
    ...
    const players = await dataService.getRetiredPlayers({teamId});
