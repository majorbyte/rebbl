...
class BB3Service{
  ...
  updateCoaches = async () => (await accountService.getBB3Accounts()).filter(x => x.bb3id !== "").map(x => this.#updateCoachInfo(x.bb3id));

  async #updateCoachInfo(coachId){
    const data = await apiService.gamerInfo(coachId);
    if (data == null || data.gamers.gamer == null) return;
    ...
    await accountService.updateBB3Info(coachId, info);
  }
  ...
  async updateSchedules (competition, day){
    ...
    let response;
      response = await axios.get(`${this.apiUrl}/api/competition/schedule/${competition.id}/${day}`);
  ...
  async updateCompetitions (leagueId){
    try{
      ...
      let competitions = response.data.responseGetCompetitions.competitions.competition.map(mapCompetition);
  ...
  async advanceCompetition(competitionId){
    ...
    const canAdvance = schedules.data.responseGetCompetitionSchedule.schedule.contest.every(x => x.matches.match.status === "3" || x.matches.match.status === "4" );
  ...
  async checkRetiredPlayers(){
    ...
    for(const teamId of teamIds){
      try{
        ...
        response = await axios.get(`${this.apiUrl}/api/competition/team/${teamId}`);

        //team is not in a comp, between seasons.
        if (!response.data.responseGetTeamCompetitions.competitions.competition) return;

        const competitionId = response.data.responseGetTeamCompetitions.competitions.competition.id;
  ...
  async #downloadReplay(schedule){
    ...
    let response = await axios.get(`${this.apiUrl}/api/cabaltv/${match.gameId}/download`); 
    ...
    response = await axios.get(`${this.apiUrl}/api/statistics/dice/${match.gameId}`); 
    if (!response.data.responseGetMatchDiceRolls) return;

    response.data.responseGetMatchDiceRolls.matchDiceRolls.matchId = match.matchId;
    response.data.responseGetMatchDiceRolls.matchDiceRolls.gameId = match.gameId;
    
    await dataService.updateDice({matchId:match.matchId,gameId:match.gameId},response.data.responseGetMatchDiceRolls.matchDiceRolls,{upsert:true});
  }
