...
class BB3Service{
  ...
  async searchTeams(coachId,teamName){
    ...
    this.#ensureArray(responseGetTeams.teams.team); //if (!Array.isArray(responseGetTeams.teams.team)) responseGetTeams.teams.team = [responseGetTeams.teams.team];
  ...
  async getRanking(competitionId){
    ...
    this.#ensureArray(responseGetCompetitionRanking.entries.competitionRankingEntry); //if (!Array.isArray(responseGetCompetitionRanking.entries.competitionRankingEntry)) responseGetCompetitionRanking.entries.competitionRankingEntry = [responseGetCompetitionRanking.entries.competitionRankingEntry];
    ...
    while (x < ranking.total){
      ...
      this.#ensureArray(responseGetCompetitionRanking.entries.competitionRankingEntry); //if (!Array.isArray(response.data.responseGetCompetitionRanking.entries.competitionRankingEntry)) response.data.responseGetCompetitionRanking.entries.competitionRankingEntry = [response.data.responseGetCompetitionRanking.entries.competitionRankingEntry];
      ranking.entries.push(...responseGetCompetitionRanking.entries.competitionRankingEntry.map(mapEntry));
      x += 50;
    }
  ...
  async updateSchedules (competition, day){
    ...
    this.#ensureArray(responseGetCompetitionSchedule.schedule.contest);// if (!Array.isArray(response.data.responseGetCompetitionSchedule.schedule.contest)) response.data.responseGetCompetitionSchedule.schedule.contest = [response.data.responseGetCompetitionSchedule.schedule.contest];
    ...
    for(const schedule of schedules){
      ...
      if(!scheduleExists || existingSchedule.status != schedule.status || homeTVChanged || awayTVChanged || teamChangedButNotPlayed){
        ...
        if (competition.parentId){
          delete schedule.id;
          schedule.competitionName = competition.displayName || competition.name;
          //schedule.round = parseInt(competition.name.split(" ").pop().replace("R", ""));
          schedule.round += competition.offsetRound;
  ...
  async calculateLeagueStandings(leagueId){
    ...
    const competitions = await dataService.getCompetitions({leagueId:leagueId,season:"season 3"});
  ...
  async updateTeams(leagueId){
    ...
    for(const competition of competitios){
      ...
      if (!response.ok) continue;
      const responseGetCompetitionSchedule = (await response.json()).responseGetCompetitionSchedule;
      if (!Array.isArray(responseGetCompetitionSchedule.schedule.contest)) continue;

      for(const schedule of responseGetCompetitionSchedule.schedule.contest){
  ...
  async getUpcomingMatch(_redditUser,_coachId, user=String(_redditUser), coachId=String(_coachId)){
    //let s = await dataService.getSchedules({gameId:{$in:["30e2812a-f799-11ef-a124-bc2411305479","fb3fe998-ff84-11ef-a124-bc2411305479"]}});
    //return [await this.#unplayedMatch(s[0]),await this.#unplayedMatch(s[1])];
    ...
    schedules = await dataService.getSchedules({"home.coach.id": coachId, "status":1} ); //1 == scheduled
    schedules = schedules.concat( await dataService.getSchedules({"away.coach.id": coachId, "status":1} ));
