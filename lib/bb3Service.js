...
class BB3Service{
  ...
  async calculateLeagueStandings(leagueId){
    ...
    const competitions = await dataService.getCompetitions({leagueId:leagueId,season:"season 4"});
  ...
  async getUpcomingMatch(_redditUser,_coachId, competitionId){
    ...
    schedules = await dataService.getSchedules({"home.coach.id": coachId, "status":1, competitionId}); //1 == scheduled
    schedules = schedules.concat( await dataService.getSchedules({"away.coach.id": coachId, "status":1,competitionId} ));
