...
class SignUpService {
  ...
  async getSignUp(redditName, type){
    return await dataService.getSignup({'reddit': redditName, type:type,season:"season 23"});
  }
  ...
  async checkTeams(search){
    const data = await dataService.getSignups(search || {season:"season 23",teamId:{$exists:0}});
