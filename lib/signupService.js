...
class SignUpService {
  ...
  async getSignUp(redditName, type, season = "season 2"){
    return await dataService.getSignup({'reddit': redditName, type:type,season});
  }
  ...
  async checkTeams(search){
    const data = await dataService.getSignups(search || {season:"season 2",teamId:{$exists:0}});
    ...
    let queue = async.queue(async function(signUp, cb){
      ...
      update({'_id':signUp._id}, {$set:signUp});
      cb();
    }, 20 /* 20 at a time*/);
  ...
  async saveSignUpBB3(signUp){
    signUp.signupDate = new Date(Date.now());
    dataService.updateSignup({'reddit':signUp.reddit, season:"season 2", type:"rebbl3"}, {$set: signUp}, {upsert:true});
    return signUp;

  }
  async saveSignUpBB3Rookies(signUp){
    signUp.signupDate = new Date(Date.now());
    dataService.updateSignup({'reddit':signUp.reddit, season:"season 2", type:"rebbrl3"}, {$set: signUp}, {upsert:true});
    return signUp;
  ...
  async resign(redditName,type, season = "season 2"){
    let signUp = await this.getSignUp(redditName,type,season);
    if(signUp){
      await dataService.deleteSignup({'reddit':redditName, type:type, season:season});
      this.notifyClients();
    }
  }
