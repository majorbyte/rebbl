...
class SignUpService {
  ...
  async getSignUp(redditName, type, season = "season 25"){
  ...
  async checkTeams(search){
    const data = await dataService.getSignups(search || {season:"season 25",teamId:{$exists:0}});
  ...
  async saveSignUpBB3(signUp){
    signUp.signupDate = new Date(Date.now());
    dataService.updateSignup({'reddit':signUp.reddit, season:"season 1", type:"rebbl3"}, {$set: signUp}, {upsert:true});
    return signUp;
  ...
  async resign(redditName,type, season = "season 25"){
