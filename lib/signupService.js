...
class SignUpService {
  ...
  async getSignUp(redditName, season = "season 3"){
  ...
  async checkTeams(search){
    const data = await dataService.getSignups(search || {season:"season 3",teamId:{$exists:0}});
  ...
  async saveSignUpBB3(signUp){
    signUp.signupDate = new Date(Date.now());
    dataService.updateSignup({'reddit':signUp.reddit, season:"season 3"}, {$set: signUp}, {upsert:true});
    return signUp;
  ...
  async resign(redditName, season = "season 3"){
