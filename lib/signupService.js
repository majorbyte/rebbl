...
class SignUpService {
  ...
  async getExistingTeam(redditName) {
    ...
    if (coach){
      ...
      let leagues = await dataService.getSchedules({$and: [{"opponents.coach.name":{$regex:coachName},league:/gman|rel|big/i, "opponents.team.name":{$not:/^\[admin/i}}]},{},{match_uuid:-1}, 10);
