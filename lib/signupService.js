...
class SignUpService {
  ...
  async callHook(signup){
    ...
    if (signup.league.toUpperCase() === "REL"){
      ...
      //ping eye
      axios.post(url, JSON.stringify({content: "<@312308842098589696>"}),{headers:{"Content-Type": "application/json"}}).catch(error => {
        console.log(error.response);
      });
    } 
  ...
  async getExistingTeam(redditName) {
    ...
    if (coach){
      let coachName = new RegExp(coach.coach,'i');
      let leagues = await dataService.getSchedules({"opponents.0.coach.name":{$regex:coachName},league:/gman|rel|big/i, "opponents.0.team.name":{$not:/^\[admin]/i}},{},{match_uuid:-1}, 10);
      ...
      coach.team = league.opponents[0].team.name;
      coach.race = league.opponents[0].team.race;
    }
    return coach;
  }

  async getSignUp(redditName, type){
    return await dataService.getSignup({'reddit': redditName, type:type,season:"season 15"});
  }
