...
class TourneyService{
  ...
  async getContests(){
    ...
    try {
      ...
      let matches = await cyanideService.matches({platform:"pc","id_only":1, start:data, league:"Huge Jackedman Memorial"}); //
      if (!matches) return;
  ...
  async calculateStandingsHJMC(){
    let schedules = await dataService.getSchedules({league:"HJMC",format: "round_robin", season:"Cup 6"});
