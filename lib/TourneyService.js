...
class TourneyService{
  ...
  async _parseNewContest(match){
    try {
      if (match.league === "Huge Jackedman Memorial") match.league = "HJMC";
  ...
  async getContests(){
    ...
    try {
      ...
      let matches = await cyanideService.matches({platform:"pc","id_only":1, start:date, league:"Huge Jackedman Memorial"}); //Huge Jackedman Memorial
      if (!matches) return;
  ...
  async calculateStandingsHJMC(){
    let schedules = await dataService.getSchedules({league:"HJMC",format: "round_robin", season:"Cup 3"});
    ...
    for(let x=0;x<l;x++){
      let matches = await dataService.getMatches({"match.leaguename":"Huge Jackedman Memorial","match.competitionname":competitions[x]});
      ...
      */
      ...
      await Promise.each(matches, async match => {
        ...
        let homeCoach = this._getCoach(coaches,0,competitions[x],match.match);
        let awayCoach = this._getCoach(coaches,1,competitions[x],match.match);
      ...
      if (coaches.length > 0){
        await dataService.removeStandings({"league": new RegExp("HJMC","i"), "competition":competitions[x], "season":"Cup 6"});
  ...
  _getCoach(coaches, index, competition, match){
    let coach = coaches.find(function (c) {
      return c.id === match.coaches[index].idcoach && c.competition == competition
    });

    if (!coach) {
      coach = this.newScore("HJMC",competition,"Cup 6",match.coaches[index],match.teams[index],0,0); 
      coaches.push(coach);
    }
  ...
  _processRoster(roster, raceId){
      ...
      for(let x=0;x<guys.length;x++){
        ...
        if (!bonusPointsSPP){
          /*• 2 bonus points if your Big Guy (not applicable to Ogre's as noted above, but this is applicable to Halflings, Goblins and Khemri) 
                ...
                right and Majorbyte can suck it
          */
          let players = roster.filter(p => p.xp_gain >= spp);
          if (players.length === 1){
            points += 2;
            bonusPointsSPP = true;
          }
    ...
    if(raceId === ogreRaceId || raceId ===khemriRaceId){
      guys = roster.filter(x => ["Khemri_TombGuardian","Ogre_Ogre"].indexOf(x.type) > -1 );
      points -= (guys.length -1);
    }
    return points;
  }
