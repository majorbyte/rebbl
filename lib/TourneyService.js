  ...
  async getContests(){
    ...
    try {
      ...
      Promise.each(data, async m => {
        m.match.competitionname = `Cup 7 - ${m.match.competitionname}`; 
      ...
      await Promise.each(rounds, async round => 
        await Promise.each(divisions, async division=>{
          options.competition = division.replace("Cup 7 - ", "");
          options.round = round;
    ...
    await this.calculateStandingsHJMC("HJMC", "Cup 7", "Huge Jackedman Memorial");
    return false;
  }
