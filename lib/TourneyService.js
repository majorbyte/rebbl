  ...
  async getContests(){
    ...
    try {
      ...
      Promise.each(data, async m => {
        m.match.competitionname = `Cup 8 - ${m.match.competitionname}`; 
      ...
      await Promise.each(rounds, async round => 
        await Promise.each(divisions, async division=>{
          options.competition = division.replace("Cup 8 - ", "");
          options.round = round;
    ...
    await this.calculateStandingsHJMC("HJMC", "Cup 8", "Huge Jackedman Memorial");
    return false;
  }
