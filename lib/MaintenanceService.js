...
class MaintenanceService{
    ...
    async _parseContest(match, swiss, additional){
      ...
      if ( match.contest_id <= 187525 ){
        ...
        await dataService.insertSchedule(match); 
     console.log(match.competition);
      } else if(contest.manual) {
        return;
      } else if (match.status === "played" && contest.match_uuid === null) {
        await dataService.updateSchedule({contest_id: match.contest_id}, match);
        ...
        await dtaService.updateSchedule({contest_id: match.contest_id}, match);
