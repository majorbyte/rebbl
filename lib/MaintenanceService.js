  ...
  async init(socket){
    ...
    async _parseContest(match, swiss, additional, skipMatch){
      ...
      if(swiss) {
        ...
        match.old_comp_id = match.competition_id;
        match.competition = swiss.comp;
        match.competition_id = swiss.id;
      ...
      if (additional){
        ...
        match.competition_id = additional.id;
          ...
          await Promise.each(season.leagues, async league=> {
            ...
            for(let competition of data.competitions) {
              ...
              if (competition.round > 1 || league.name === "RAMPUP"){
                ...
                if (contests && contests.contests){ 
                  await Promise.all(contests.contests.map(async function(contest){
              ...
              if (contests && contests.contests){
                ...
                if (contests && contests.contests && contests.contests.length > 0){
                  if( swiss) {
                    division = swiss.comp;
                  }
    ...
    async getContests(leagueName){
      ...
      for(let season of seasons){
        for(let league of season.leagues){
          ...
          if(!contests || !contests.contests) continue;

          let upcoming_matches = contests.contests;
          ...
          if(league.format && league.format === "swiss"){
            ...
            if (Array.isArray(scheduled.contests)){
              for(let c of scheduled.contests){
          ...
          if (league.combine){
            ...
            if(swiss && swiss.contests) upcoming_matches = upcoming_matches.concat(swiss.contests);
          }
          ...
          for(let contest of upcoming_matches){
            ...
            if (league.combine){
              ...
              if (div){
                ...
                contest.old_comp_id = contest.competition_id;
                //fix competition name
                contest.competition = div.comp;
                contest.competition_id = div.id;
                //fix round
                contest.round = contest.round + offset;
              }
            }
          ...
          for(const update of toUpdate){
            update.calc === "hjmc" ? await tourneyService.calculateStandingsHJMC(update.league, update.season ,update.league) : await standingsService.updateStandings(update.season, update.league, update.competition);
          }
    ...
    async fixAdminGames(contestId){
      ...
      let matches = contests.contests.map(x => x.match_uuid);
      let found = await dataService.getMatches({"uuid":{$in:matches}},{"uuid":1});
      found = found.map(f => f.uuid);
      ...
      await Promise.each(data, async m => await teamService.updateTeamsAfterMatch(m));
      await Promise.each(contests.contests, async c => await this._parseNewContest(c));
      await standingsService.updateStandings(contest.season, contest.league, contest.competition);
    ...
    async updateLegendaryPlayers(){
      ...
      await dataService.removeLegendayPlayers({});
      dataService.insertLegendayPlayers(data);
    }
