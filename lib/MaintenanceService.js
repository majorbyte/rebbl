  ...
  async init(socket){
    ...
    async getRebblData(leagueName){
      ...
      switch(leagueName){
        ...
        case "greenhorn":
          ...
          break;
        case "rookies":
          seasons = [configurationService.getActiveCollegeSeason()];          
          break;
        default:
          seasons = [configurationService.getActiveOneMinuteSeason()];
    ...
    async fixAdminGames(contestId){
      ...
      let season = configurationService.getSeasons()
        .find(x => x.active && x.leagues.filter(l => l.name.toLowerCase() === contest.league.toLowerCase()));
      ...
      if(combined){
        let x = 0;
        let combine = combined.additional[x];
        while (combine.offset < contest.round-1){
          combine = combined.additional[x];
          x++
        }
        options = {platform:"pc", competition:combine.name, league:contest.league,round:contest.round-combine.offset, status:"played", exact: 1, limit:20};
      } else {
        options = {platform:"pc", competition:contest.competition, league:contest.league,round:contest.round, status:"played", exact: 1, limit:20};
      }
