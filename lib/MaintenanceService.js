...
class MaintenanceService{
  async init(socket){
    ...
    async _parseNewContest(match, swiss, additional, calculationMethod){
      ...
      try {
        ...
        if (!contest) {
          ...
          dataService.updateSchedule({contest_id: match.contest_id}, match);
//          await calculationMethod === "hjmc" ? tourneyService.calculateStandingsHJMC(match.league, match.season ,match.league) : standingsService.updateStandings(match.league, match.competition);
    ...
    async getContests(leagueName){
      ...
      for(var season of seasons){
        for(var league of season.leagues){
          ...
          await Promise.each(upcoming_matches, async contest =>{
            let swiss = null;
          ...
          //update standings
          await Promise.each(toUpdate, async update =>{
            await update.calc === "hjmc" ? tourneyService.calculateStandingsHJMC(update.league, update.season ,update.league) : standingsService.updateStandings(update.league, update.competition);
          });
        ...
        for (let seasonIndex = 0; seasonIndex<seasons.length;seasonIndex++){
          ...
          for(let leagueIndex=0;leagueIndex<leagues.length;leagueIndex++){
            ...
            for(let divisionIndex=0;divisionIndex<divLength;divisionIndex++){
              ...
              let exact = league.hasOwnProperty("exact") ? (league.exact ? 1 : 0) : 1;
