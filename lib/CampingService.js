...
/*
    ...
    New Sheriff In Town - Beat any Division 1 team from Season 13
    Flat Track Bully - Beat any Division 5 team from Season 13
...
*/
  ...
  class CampingService{
    ...
    async updateBadges(){
      ...
      let matches = await dataService.getMatches({"match.competitionname":/^REBBL Summer Camp$|^SC500/});
      let coaches = [];
    ...
    async _processBadges(coaches, match){
      let homeCoach = await this._getCoach(coaches,0,"REBBL Off Season","REBBL Summer Camp","season 1",match);
      let awayCoach = await this._getCoach(coaches,1,"REBBL Off Season","REBBL Summer Camp","season 1",match);
      ...
      if (match.teams[0].score >= match.teams[1].score){
        ...
        homeCoach.badges.newSheriffInTown = homeCoach.badges.newSheriffInTown || /Season 13.*Division 1/i.test(awayCoach.mainCompetition);
        ...
        awayCoach.badges.newSheriffInTown = awayCoach.badges.newSheriffInTown || /Season 13.*Division 1/i.test(homeCoach.mainCompetition);
    ...
    async _newCoach(league, comp, season, coach, team){
      ...
      let coachData = await this._getCoachLeagueCompetition(/^rebbl/i,coach.idcoach,/season 13/i);
      if (!coachData)
        coachData = await this._getCoachLeagueCompetition(/^rebbrl minors/i,coach.idcoach,/season 9/i);
      
      if (!coachData && coach.coachname === "MisspelledTree"){
        coachData= coach;
        coachData.competition = "Season 13 Division 2";
        coachData.league = "REBBL - Big O";
      } 
