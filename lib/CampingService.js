...
*/
  ...
  class CampingService{
    ...
    async updateBadges(){
      ...
      let matches = await dataService.getMatches({"match.competitionname":/^REBBL Summer Camp$|^SC500/,"match.started":{$gt:"2020-08-01"}});
      let coaches = [];
    ...
    async _processBadges(coaches, match){
      let homeCoach = await this._getCoach(coaches,0,"REBBL Off Season","REBBL Summer Camp","season 2",match);
      let awayCoach = await this._getCoach(coaches,1,"REBBL Off Season","REBBL Summer Camp","season 2",match);
      ...
      if (match.teams[0].score >= match.teams[1].score){
        ...
        homeCoach.badges.newSheriffInTown = homeCoach.badges.newSheriffInTown || /Season 14.*Division 1/i.test(awayCoach.mainCompetition);
        ...
        awayCoach.badges.newSheriffInTown = awayCoach.badges.newSheriffInTown || /Season 14.*Division 1/i.test(homeCoach.mainCompetition);
    ...
    async _newCoach(league, comp, season, coach, team){
      ...
      let coachData = await this._getCoachLeagueCompetition(/^rebbl/i,coach.idcoach,/season 14/i);
      if (!coachData)
        coachData = await this._getCoachLeagueCompetition(/^rebbrl minors/i,coach.idcoach,/season 10/i);
