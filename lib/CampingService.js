...
*/
  ...
  class CampingService{
    ...
    async _processBadges(coaches, match){
      ...
      homeCoach.badges.discountArmour = homeCoach.badges.discountArmour + match.teams[0].roster.reduce((total, current) => total += current.sustainedtackles, 0); 
      awayCoach.badges.discountArmour = awayCoach.badges.discountArmour + match.teams[1].roster.reduce((total, current) => total += current.sustainedtackles, 0);  
