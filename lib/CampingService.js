...
*/
  ...
  class CampingService{
    ...
    async updateBadges(){
      ...
      let matches = await dataService.getMatches({"match.competitionname":/^REBBL Winter Camp$|^WC500/,"match.started":{$gt:"2022-02-01"}});
      let coaches = [];
    ...
    async _processBadges(coaches, match){
      let homeCoach = await this._getCoach(coaches,0,"REBBL Off Season","REBBL Winter Camp","season 4",match);
      let awayCoach = await this._getCoach(coaches,1,"REBBL Off Season","REBBL Winter Camp","season 4",match);
      ...
      if (match.teams[0].score >= match.teams[1].score){
        ...
        homeCoach.badges.newSheriffInTown = homeCoach.badges.newSheriffInTown || (/Season 18.*Division 1/i.test(awayCoach.mainCompetition));
        ...
        awayCoach.badges.newSheriffInTown = awayCoach.badges.newSheriffInTown || (/Season 18.*Division 1/i.test(homeCoach.mainCompetition));
    ...
    async _newCoach(league, comp, season, coach, team){
      ...
      let coachData = await this._getCoachLeagueCompetition(/^rebbl/i,coach.idcoach,/season 18/i);
      if (!coachData)
        coachData = await this._getCoachLeagueCompetition(/^rebbrl minors/i,coach.idcoach,/season 14/i);
