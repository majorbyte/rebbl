...
class WorldCupQualifierService{
  constructor(){
    this.matches = new Datastore.datastore('datastore/other/wcq2018/matches.db');
    this.schedule = new Datastore.datastore('datastore/other/wcq2018/schedule.db');
    this.coaches = new Datastore.datastore('datastore/other/wcq2018/coaches.db');
    this.teams = new Datastore.datastore('datastore/other/wcq2018/teams.db');
  ...
  async getLeagueData(round){
    try {
      ...
      for(var i=0;i<leagues.length;i++){    
        const contests = await cyanideService.contests({league : leagues[i], status:"*", exact: 1, v: 1,limit:150});
...
//
  ...
  async getMatchesForCoach(_id, coachName=String(_id)){
    let coach = await this.coaches.findOne({name: coachName});
    if (coach) {
      return await this.schedule.find({"opponents.coach.id" : coach.id});
    }
    return [];
  }
...
module.exports = new WorldCupQualifierService();