...
class BB3MatchReport{
  ...
  async matchReport (uuid, hookUrl){
    ...
    let embed = {
      ...
      [${match.competition.name}](https://rebbl.net/competition/${match.competition.id}/standings)
      `,
      "color": 187908,
      "thumbnail": {
    ...
    await Promise.all(hooks.map(hook => fetch(hook.url, { method:"POST", headers:{"Content-Type": "application/json"}, body: body(hook)} )
        .catch(async error => {
