...
class BB3MatchReport{
  ...
  matchReport = async function(uuid, hookUrl){
    ...
    let embed = {
      ...
      "fields": [
        ...
        {
          "name": "**injuries home**",
          "value": this.getInjuries(match.statistics, 1),
          "inline": true
        },
        {
          "name": "**injuries away**",
          "value": this.getInjuries(match.statistics, 0),
          "inline": true
        }
      ]
    };  
    ...
    await dataService.updateMatch({gameId:uuid},{$set:{reported:true}});
  }
  ...
  getPlayerText = player => `${player.name } *${this.getPosition(player.position)}*
  **${this.getCasualties(player.casualties)}** (${player.spp} SPP)
  `;
