...
class BB3MatchReport{
  ...
  matchReport = async function(uuid, hookUrl){
    ...
    const match = await dataService.getMatch({gameId:uuid});  
    if (!match)return;
    ...
    let embed = {
      ...
      "fields": [
        {
          "name": "**gamestats**",
          "value": getStats(match.statistics),
          "inline": true
        },
        {
          "name": "**injuries home**",
          "value": getInjuries(match.statistics, 1),
          "inline": true
        },
        {
          "name": "**injuries away**",
          "value": getInjuries(match.statistics, 0),
          "inline": true
        }
      ]
    };  
  ...
  getStats = function(statistics){
    return `\`\`\`python
        home away
  TD       ${getStatistic(statistics,18)}     
  BLK     ${getStatistic(statistics,26)}
  AVBr     ${getStatistic(statistics,61)}
  KO+      ${getStatistic(statistics,25)}
  CAS+     ${getStatistic(statistics,19)}
  CAS-     ${getStatistic(statistics,28)}
  Kills    ${getStatistic(statistics,21)}
  Death    ${getStatistic(statistics,31)}
  \`\`\`
     `  
  }

  getStatistic = function(stats, id){
    if (!Array.isArray(stats.homeGamerStatistics.teamStatistics.statistics.aggregatedStatistics)) stats.homeGamerStatistics.teamStatistics.statistics.aggregatedStatistics = [stats.homeGamerStatistics.teamStatistics.statistics.aggregatedStatistics];
    if (!Array.isArray(stats.awayGamerStatistics.teamStatistics.statistics.aggregatedStatistics)) stats.awayGamerStatistics.teamStatistics.statistics.aggregatedStatistics = [stats.awayGamerStatistics.teamStatistics.statistics.aggregatedStatistics];
    const home = stats.homeGamerStatistics.teamStatistics.statistics.aggregatedStatistics?.find(x => Number(x.statId) == id);
    const away = stats.awayGamerStatistics.teamStatistics.statistics.aggregatedStatistics?.find(x => Number(x.statId) == id);
    return `${home?.value || 0}  ${away?.value || 0}`
  }

  getInjuries = function(statistics, home){
    const s = home ? statistics.homeGamerStatistics : statistics.awayGamerStatistics;
  
    let deadPlayers = s.teamStatistics.playerStatistics.playerStatistics.filter(x => x.player.dead == "1").map(x => x.player);
    let mngPlayers = s.teamStatistics.playerStatistics.playerStatistics.filter(x => x.player.missNextGame == "1").map(x => x.player);
    let text = `__**${s.teamStatistics.team.name}**__
    ${deadPlayers.length > 0 ? deadPlayers.map(getPlayerText).join('\n')+'\n' : ''}${mngPlayers.map(getPlayerText).join('\n')}`;
  
    return text;
  }  

  getLogo = match => match.homeScore > match.awayScore 
    ? `https://cdn2.rebbl.net/images/bb3/Logos/${match.homeTeam.logo.icon}`
    : match.homeScore < match.awayScore 
      ? `https://cdn2.rebbl.net/images/bb3/Logos/${match.awayTeam.logo.icon}`
      :"https://cdn2.rebbl.net/images/cards/fans_small.png";
  

  getPlayerText = player => `${player.name } *${getPosition(player.position)}*
  **${getCasualties(player.casualties)}** (${player.spp} SPP)
  `;
