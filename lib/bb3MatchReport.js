...
class BB3MatchReport{
  ...
  async matchReport (uuid, hookUrl){
    ...
    let embed = {
      ...
      "url":`https://rebbl.net/bb3/match/${match.gameId}`,
    ...
    let urls = webhooks.filter(webbhook => webbhook.races.some(race => race == match.homeTeam.race || race == match.awayTeam.race)).map(x => x.url);

    urls = urls.concat(webhooks.filter(webbhook => webbhook.competitions.some(competition => /\/(.+)\/.*/.test(competition) ? this.#stringToRegex(competition).test(match.competition.name) : competition == match.competition.name)).map(x => x.url));

    urls = urls.concat(webhooks.filter(webbhook => webhooks.bb3coach && (match.awayGamer.id == webbhook.bb3coach || match.homeGamer.id == webbhook.bb3coach)).map(x => x.url));
    ...
    await Promise.all(urls.map(url => axios.post(url, body,{headers:{"Content-Type": "application/json"}})))
    .then(async _ => {
      await dataServiceBB3.updateMatch({matchId:uuid},{$set:{reported:true}});
    })
    .catch(error => {
