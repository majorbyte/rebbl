...
class AdminMatchService{
  ...
  async resetMatch(matchId){
    let schedules = await dataService.getSchedules({matchId:matchId});
  ...
  async mockConcedeMatch(matchId, winningTeamId){
    let schedules = await dataService.getSchedules({matchId:matchId});
    const schedule = schedules[schedules.length-1];
    ...
    const awardByeWeekCash = async function(team,otherDedicatedFans){
    ...
    const awardByeWeekSpp = async function(team){
      // 2 MVP
      // 1 TD
    ...
    if (winningTeamId == homeTeam.id) {
      homeWinnings = await awardByeWeekCash(homeTeam, awayDF);
      await awardByeWeekSpp(homeTeam);
      homeWinnings.score = 1;
    } else {
      awayWinnings = await awardByeWeekCash(awayTeam, homeDF);
      await awardByeWeekSpp(awayTeam);
      awayWinnings.score = 1;
    }

    await this.#createMatch(competition, schedule, homeTeam, awayTeam, homeWinnings, awayWinnings);
  ...
  async concedeMatch(matchId, winningTeamId){
    const schedule = await this.#getCorrectSchedule(matchId);
  ...
  async adminDrawMatch(matchId, noScheduleAttempt){
    ...
    let cookie = null;
    let homeWinnings = {treasury: homeTeam.treasury, winnings:0,fans:homeDF+d1, fansRoll:0, score: noScheduleAttempt ? 0 : 1};
    let awayWinnings = {treasury: awayTeam.treasury, winnings:0,fans:awayDF+d2, fansRoll:0, score: noScheduleAttempt ? 0 : 1};
  ...
  async #getCorrectSchedule(matchId){
    let schedules = await dataService.getSchedules({matchId:matchId});
    ...
    if (schedules.length == 1) 
    {
      ...
      const competitionIds = schedules.map(x => x.competitionId);
    
      let competitions = await dataService.getCompetitions({id:{$in:competitionIds}});

      competitions = competitions.filter(x => x.parentId);

      const competition = competitions[competitions.length-1];

      schedule = schedules.find(x => x.competitionId == competition.id);
    }

    if (schedule.handlingByeWeek) throw(`This match (${matchId}) is already (being) processed`);

    await dataService.updateSchedule({id:schedule.id},{$set:{handlingByeWeek:true}});
  ...
  async #awardSpp(cookie, team, numberOfTouchdowns){
    ...
    while(numberOfTouchdowns>0){
      ...
      team.roster.find(x => x.player.id === viablePlayers[td-1].id).player.xpGained += 3;
      sppPlayers.push(viablePlayers[td-1]);
  ...
  async #clearMissingNextGame(cookie, team){
    const viablePlayers = team.roster.filter(x => x.missNextGame);
  ...
  #resultGain(treasury, winnings, fans, fansRoll){
    return {
      "newTreasury" : `${Number(treasury) + Number(winnings)}`,
      "newDedicatedFans" : `${fans}`,
      "previousTreasury" : `${treasury}`,
      "dedicatedFansRoll" : `${fansRoll}`,
      "fanAttendance" : `${Number(fans)+Number(fansRoll)}`,
  ...
  #mapPlayer = (player) => {
      return {
        ...
        missNextGame: player.suspended_next_match,
        type: player.type,
        mvp: player.mvp,
        level: player.level,
        xp: player.xp,
