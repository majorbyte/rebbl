...
class ApiService{
  ...
  async findCoach(name){
      ...
      var response = await axios.get(`${this.apiUrl}/api/coach/${name}/search`,{
        headers:{"Authorization": `Bearer ${this.token}`}
      });
  ...
  async createCompetition(leagueId, name, ownerId, teamCount, competitionType, turnDuration, aging, enhancement,resurrection,customTeams,mixedTeams,experiencedTeams ){
    let data = {
      "leagueId":leagueId,
      "name":name,
      "ownerId":ownerId,
