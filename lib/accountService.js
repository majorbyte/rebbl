...
class AccountService {
  ...
  async getBB3Account(name){
    try{
      ...
      let response = await axios.get(`${this.apiUrl}/api/gamer/${name}/search/${pageSize}/${pageNumber*pageSize}`);
      ...
      if (Array.isArray(gamers)){
        while (Number(response.data.responseGetGamersWithName.nbPages) > pageNumber && pageNumber < 5 ){
          pageNumber++;
          response = await axios.get(`${this.apiUrl}/api/gamer/${name}/search/${pageSize}/${pageNumber*pageSize}`);

          gamers = gamers.concat(response.data.responseGetGamersWithName.gamers.gamer);
        }

        var filtered = gamers
          .filter(gamer => gamer.name.localeCompare(name, "en-US", { sensitivity: "base" }) === 0)
          .map(gamer => {return {name:gamer.name, id:gamer.id, steamId: gamer.account.authServices.account_authServices.uuid}});


        const regex = /<title>Steam Community :: (.*)<\/title>/;

        for(let gamer of filtered){
          const response = await axios.get(`https://steamcommunity.com/profiles/${gamer.steamId}`);
          delete gamer.steamId;
          gamer.steamName = regex.exec(response.data)[1];
        }

        return filtered;
      } else {
        return {name: gamers.name, id:gamers.id};
      }
  ...
  async updateBB3Account(reddit,bb3){
    let regex = new RegExp(`^${reddit}$`, 'i');
    dataService.updateAccount({'reddit': {"$regex": regex}}, {$set:{"bb3coach":bb3.name, "bb3id":bb3.id}});
  }
