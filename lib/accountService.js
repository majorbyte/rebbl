    ...
    try{
      ...
      if (Array.isArray(gamers)){
        ...
        let filtered = gamers
          ...
          .map((gamer,index) => {return {index, name:gamer.name, id:gamer.id, steamId: gamer.account.authServices.account_authServices.uuid}});
        ...
        for(let gamer of filtered){
          ...
          if (!result.some(x => x.id == gamer.id)) result.push(gamer);
        }
