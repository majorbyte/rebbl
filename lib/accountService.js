...
const apiBB3Service = require("./apiBB3Service.js");
...
class AccountService {
  ...
  async getBB3Account(name){
    try{
      ...
      let response = await fetch(`${this.apiUrl}/api/gamer/${name}/search/${pageSize}/0`);

      if (!response.ok) return null;

      let data = await response.json();

      if (data.responseGetGamersWithName.total ==0) return null;

      let gamers = data.responseGetGamersWithName.gamers.gamer;

      if (Array.isArray(gamers)){
        while (Number(data.responseGetGamersWithName.nbPages) > pageNumber && pageNumber < 5 ){
          pageNumber++;
          response = await fetch(`${this.apiUrl}/api/gamer/${name}/search/${pageSize}/${pageNumber*pageSize-1}`);
          if (!response.ok) break;

          data = await response.json();
          gamers = gamers.concat(data.responseGetGamersWithName.gamers.gamer);
        }
  ...
  async updateBB3Account(reddit,bb3){
    ...
    const gamer = (await apiBB3Service.gamerInfo(bb3.id)).gamers.gamer;
    const info = {
    ...
    dataService.updateAccount({bb3id:bb3.id}, {$set:{info}});
  }

  async updateBB3Info(bb3id, info){
    dataService.updateAccount({bb3id}, {$set:{info}});
  }
  async clearBB3Info(bb3id){
    dataService.updateAccount({bb3id}, {$unset:{info:true, bb3coach:true, bb3id:true, bb3service:true, bb3displayId:true}});
  }
