...
class DataService{
    ...
    async init(database){
        ...
        this.scheduleTemplates = this.db.collection("scheduleTemplates");
        this.legendaryPlayers = this.db.collection("legendaryplayers");
        this.hooks = this.db.collection("hooks");
...
/* Configuration */    
    ...
    async getSeasons(param,options){
      return await this.seasons.find(param,options).toArray();
  }
...
/* Matches */    
    getMatchesChain(param,options){
      return this.matches.find(param,options);
    }
...
/* Signups */
    async getSignup(param){
      param.season ??= this.currentSeason;
...
/* scheduleTemplates */
    async getScheduleTemplate(param) { 
        return await this.scheduleTemplates.findOne(param);
    }
    async getScheduleTemplates(param,options) { 
        return await this.scheduleTemplates.find(param,options).toArray();
    }
    updateScheduleTemplate(param, scheduleTemplate, options){
        try{
            scheduleTemplate = this.checkAtomacity(scheduleTemplate);
            this.scheduleTemplates.updateOne(param,scheduleTemplate,options);
        }catch(err){
            console.error(err);
        }
    }
/* Legendary player */    
  async getLegendayPlayers(param,options){
    return await this.legendaryPlayers.find(param,options).toArray();
  }   

  insertLegendayPlayers(players){
    try{
        this.legendaryPlayers.insertMany(players);
    } catch(err){
        console.error(err);
    }
  }

  async removeLegendayPlayers(filter, options){
    try{
        return this.legendaryPlayers.deleteMany(filter, options);
    }catch(err){
        console.error(err);
    }
  }
    ...
    /* Hooks */
    async getHook(param) { 
        return await this.hooks.findOne(param);
    }
    async getHooks(param,options) { 
        return await this.hooks.find(param,options).toArray();
    }

    async getHooksWithUrl(){
        if (this.loadedHooks.length == 0){
            await this.refreshHooks();
        }
        return this.loadedHooks;
    }
    async refreshHooks(){
        const hooks = await this.hooks.find({active:true}).toArray();
        
        this.loadedHooks = hooks.map(hook => {
            hook.url = `https://discord.com/api/webhooks/${hook.id}/${this.#decrypt(hook.token)}`;
            return hook;
        });
    }
    ...
    async insertHook(hook, options){
        try{
            this.loadedHooks = [];
            return await this.hooks.insertOne(hook,options);
        }catch(err){
            console.error(err);
        }
    }
    updateHook(param, hook, options){
        try{
            this.loadedHooks = [];
            hook = this.checkAtomacity(hook);
            this.hooks.updateOne(param,hook,options);
        }catch(err){
            console.error(err);
        }
    }
    async removeHook(filter){
        this.loadedHooks = [];
        await this.hooks.deleteOne(filter);
    }
    async removeHooks(filter,options){
        this.loadedHooks = [];
        await this.hooks.deleteMany(filter,options);
    }
