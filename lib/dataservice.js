...
class DataHandler{
  constructor(){
    ...
    this.cyanide = new Cyanide(process.env['cyanideKey']);
    this._rounds = null;
  }
  ...
  async _calculateCoachStats (coach) {
    ...
    coach.strength = matchCount > 0 ? coach.score / matchCount : 0;
  ...
  async _updateStrengthOfSchedule(coach){
    ...
    for (let index = 0; index < matches.length; index++) {
      ...
      coach.strengthOfSchedule += opponentCoach.strength;
    }
    ...
    this.coaches.update(
      ...
      {
        $set: {
          "strengthOfSchedule": coach.strengthOfSchedule
        }
      }, {upsert: false}
    );
  }
