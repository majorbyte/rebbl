...
class Draft {
  ...
  async getDraft(userName, division, round, house){
    ...
    if (division && round && house){
      schedule = await dataService.getSchedule({season:/season 18/i,league:"clan",competition:division, round:Number(round), house: Number(house)});
    }
    else{
      if (!clan) return;
      schedule = await dataService.getSchedule({season:/season 18/i,drafted:false,league:"clan",$or:[{"home.clan":clan.name},{"away.clan":clan.name}]},{sort:{round:1},limit:1});
    }
    ...
    const draft = await dataService.getDraft({season:/season 18/i,competition:schedule.competition, round:schedule.round, house: schedule.house});
    if (draft){
  ...
  async saveDraft(userName, draft){
    ...
    let savedDraft = await dataService.getDraft({season:/season 18/i,competition:draft.competition, round:draft.round, house: draft.house});
    ...
    clan = await dataService.getClan({name:confirmByClan, season:"season 18"});
  ...
  async createAssassinationCompetition(competition, assassination){
    ...
    await this.setupCompetition(this.getPowerLeagueId(competition), name, false, assassination.team);
  }
  async createEmergencyRnRCompetition(competition, emergency){
    let name = `rnr ${emergency.team.name.substr(0,21)}`;
    await this.setupCompetition(this.getPowerLeagueId(competition), name, false, emergency.team);
  }
  async createInspirationCompetition(competition, inspiration){
    let name = `ins ${inspiration.player.name.substr(0,21)}`;
    await this.setupCompetition(this.getPowerLeagueId(competition), name, false, inspiration.team);
  }
  async createBloodSacrificeCompetition(competition, sacrifice){
    let name = `bsf ${sacrifice.team.name.substr(0,21)}`;
    await this.setupCompetition(this.getPowerLeagueId(competition), name, false, sacrifice.team);
  }
  async createContest(competition, house, round, game, contest){
    ...
    await this.setupCompetition(this.getLeagueId(competition,house),name,true, getTeam(contest.awayTeam.team), getTeam(contest.homeTeam.team));
  }


  async setupCompetition(leagueId, name, resurrection){
    const competitionId = await this.createCompetition(leagueId,name,resurrection);

    if (competitionId === -1) return;
    let teams = Array.prototype.slice.call(arguments, 3);
  ...
  async createCompetition(leagueId, name, resurrection){
  ...
  async getSchedulingPost(round) {
    ...
    let post = response.data.data.children.find(x => x.data.title.toLowerCase() === `clan season 18 week ${round}`)

    if (!post) 
    {
      config.params.q = "flair:Clan announcement";
      response = await axios(config);
      post = response.data.data.children.find(x => x.data.title.toLowerCase() === `clan season 18 week ${round}`)
    }
    return post;
  }
...
[Draft Result](https://rebbl.net/clandraft/?division=${draft.competition}&round=${draft.round}&house=${draft.house})`;
  ...
  getRedditHomeMatch(team,clan){
    ...
    return `${coach?.reddit || 'unknown'} [${team.team.name}](https://rebbl.net/team/${team.team.id}) **${this.getRace(team.team.idraces)}**`;
  }

  getRedditAwayMatch(team,clan){
    const coach = clan.members.find(m => m.coachId === team.coach.id);
    return `**${this.getRace(team.team.idraces)}** [${team.team.name}](https://rebbl.net/team/${team.team.id}) ${coach?.reddit || 'unknown'}`;
  }
