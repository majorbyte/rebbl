...
class Draft {
  ...
  async getDraft(userName, division, round, house){
    ...
    if (division && round && house){
      schedule = await dataService.getSchedule({season:/season 15/i,league:"clan",competition:division, round:Number(round), house: Number(house)});  
    }
    else{
      if (!clan) return;
      schedule = await dataService.getSchedule({season:/season 15/i,drafted:false,league:"clan",$or:[{"home.clan":clan.name},{"away.clan":clan.name}]},{sort:{round:1},limit:1});      
    }
    ...
    const draft = await dataService.getDraft({season:/season 15/i,competition:schedule.competition, round:schedule.round, house: schedule.house});
    if (draft){
  ...
  async saveDraft(userName, draft){
    ...
    let savedDraft = await dataService.getDraft({season:/season 15/i,competition:draft.competition, round:draft.round, house: draft.house});
    ...
    clan = await dataService.getClan({name:confirmByClan, season:"season 15"});
  ...
  async getSchedulingPost(round) {
    ...
    let post = response.data.data.children.find(x => x.data.title.toLowerCase() === `clan season 15 week ${round}`)

    if (!post) 
    {
      config.params.q = "flair:Clan announcement";
      response = await axios(config);
      post = response.data.data.children.find(x => x.data.title.toLowerCase() === `clan season 15 week ${round}`)
    }
    return post;
  }
