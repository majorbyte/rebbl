...
class Draft {
  ...
  async getAuthToken(){
    ...
    this.redditAuthToken = data.access_token;
  }
  ...
  async getSchedulingPost(round) {
    ...
    let response = await fetch('https://oauth.reddit.com/r/rebbl/search', options);
    if (!response.ok) return;

    let data = await response.json();
    let post = data.data.children.find(x => x.data.title.toLowerCase() === `clan season 19 week ${round}`)

    if (!post) 
    {
      response = await fetch('https://oauth.reddit.com/r/rebbl/search', options);
      if (!response.ok) return;
      data = await response.json();
      post = data.data.children.find(x => x.data.title.toLowerCase() === `clan season 19 week ${round}`)
    }
    return post;
  }
  ...
  async postSummary(fullname,text){
    ...
    const response = await fetch('https://oauth.reddit.com/api/comment', options);
    if (!response.ok) return
    const data = await response.json();
    
    const result = {fullname:data.name, permalink:data.permalink};
  ...
  async toggleReplies(fullname,state){
    ...
    await fetch('https://oauth.reddit.com/api/sendreplies', options);
  }
  ...
  async postComment(fullname,text){
    ...
    await fetch('https://oauth.reddit.com/api/comment',options);
  }
