...
class Draft {
  ...
  async #createGames(draft,data){
    ...
    await clanService.updateUnstarted(data.division,data.round,data.house);
  }
...
[Draft Result](https://rebbl.net/clandraft/?division=${draft.competition}&round=${draft.round}&house=${draft.house})`;
  ...
  getRedditUsers(draft,homeClan,awayClan){
    return draft.contests.map(contest => `${this.getRedditHomeUser(contest.homeTeam.team,homeClan)} vs. ${this.getRedditAwayUser(contest.awayTeam.team,awayClan)} schedule here`);
  }
