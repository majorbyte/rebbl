...
class Draft {
  ...
  async confirmDraft(userName, data){
    ...
    const isAdmin = await accountService.hasRole(userName, "clanadmin");
    ...
    for(const side in draft.powers){
      ...
      for(const power in powers){
        ...
        await this.registerPower(power, amount, side === "home",draft.competitionName,draft.round,draft.house);
  ...
  async getSchedulingPost(round) {
    ...
    return response.data.data.children.find(x => x.data.title === `CLAN Season 9 week ${round}`);
  }
...
${awayClan.name}: ${this.getPostDraftPowers(draft.powers.away)}`;
  ...
  getPreDraftPowers(draftPowers){
    ...
    for(const p of powers){
      if(draftPowers[p] === 0 || draftPowers[p].length === 0) continue;
      usedPowers = Array.isArray(draftPowers[p]) ? usedPowers.concat(Array(draftPowers[p].length).fill(p)) : usedPowers.concat(Array(draftPowers[p]).fill(p));
    }
