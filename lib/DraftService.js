...
class Draft {
  ...
  async getDraft(userName, division, round, house){
    ...
    if (division && round && house){
      schedule = await dataService.getSchedule({season:/season 10/i,league:"clan",competition:division, round:Number(round), house: Number(house)});  
    }
    else{
      if (!clan) return;
      schedule = await dataService.getSchedule({season:/season 10/i,drafted:false,league:"clan",$or:[{"home.clan":clan.name},{"away.clan":clan.name}]},{sort:{round:1},limit:1});      
    }
    ...
    const draft = await dataService.getDraft({season:/season 10/i,competition:schedule.competition, round:schedule.round, house: schedule.house});
    if (draft){
  ...
  async saveDraft(userName, draft){
    ...
    let savedDraft = await dataService.getDraft({season:/season 10/i,competition:draft.competition, round:draft.round, house: draft.house});
    ...
    clan = await dataService.getClan({name:confirmByClan, season:"season 10"});
  ...
  async createCompetition(leagueId, name){
    ...
    let result = await apiService.createCompetition(leagueId,name,ownerId, 2, 0,"RoundRobin","ThreeMinutes","InviteOnly",true,true,false,true,false,true,false,true);
    if (!result) result = await apiService.createCompetition(leagueId,name,ownerId, 2, 0,"RoundRobin","ThreeMinutes","InviteOnly",true,true,false,true,false,true,false,true); //try once more
  ...
  async getSchedulingPost(round) {
    ...
    return response.data.data.children.find(x => x.data.title === `CLAN Season 10 week ${round}`);
  }
