...
class Draft {
  ...
  async getDraft(userName, division, round, house){
    ...
    const draft = await dataService.getDraft({confirmed:false,competition:schedule.competition, round:schedule.round, house: schedule.house});
    if (draft){
  ...
  async confirmDraft(userName, data){
    ...
    draft.powers.home.bloodSacrifice.map(x => this.createbloodSacrificeComp(draft.competition, x));
    draft.powers.away.bloodSacrifice.map(x => this.createbloodSacrificeComp(draft.competition, x));
  ...
  async createBloodSacrificeCompetition(competition, sacrifice){
    let name = `bsf ${sacrifice.player.name.substr(0,21)}`;
