...
class Draft {
  ...
  async getSchedulingPost(round) {
    ...
    let post = response.data.data.children.find(x => x.data.title === `CLAN season 12 week ${round}`)

    if (!post) 
    {
      config.params.q = "flair:Clan announcement";
      response = await axios(config);
      post = response.data.data.children.find(x => x.data.title === `CLAN season 12 week ${round}`)
    }
    return post;
  }
