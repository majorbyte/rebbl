...
class LeagueHandler{
  ...
  async _parseContest(match, round){
    ...
    if (contest && (contest.match_uuid === null || contest.match_uuid === "1000000000")) return false; //not played yet


    return await this.getMatch(match.match_uuid, match.league, match.competition);
  }

  async getRebblData(status){
    ...
    try {
      for(let leagueIndex=0;leagueIndex<leagues.length;leagueIndex++){
        ...
        for(let divisionIndex=0;divisionIndex<divLength;divisionIndex++){
          ...
          const contests = await this.cyanide.contests({league : league.name, status:"*", exact: 1, competition: division, v:1});
